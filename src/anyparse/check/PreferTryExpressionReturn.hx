 ...
 *   one is left unflagged rather than silently losing it. A DANGLING line comment refuses the
 *   `try` even INSIDE a copied region (`TryExpressionShape.danglingLineComment`): a `//` with no
 *   newline after it within its slice comments out whatever the rebuild appends next -- the
 *   following `catch (…)` header, or the terminating `;`. A `//` EARLIER in a multi-line slice
 *   is fine, the slice being copied newlines and all.
 *
