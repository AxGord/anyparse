...
final class AvoidDynamic implements Check implements ConfigAware implements RiskyFix implements OracleAssisted implements GroupedFix {
	...
	private static function wholeDynamicLocal(
		...
		return DynamicShape.wholeDynamicDecl(tree, source, span, dynName, shape.localDeclKinds ?? []);
	}
	...
	private static function inferLocalNarrowType(
		...
		if (t == dynName || t == 'Any' || !DynamicShape.isNominalName(t)) return null;
	...
	private static function gatherUses(
		...
		final targetKeys: Map<String, Bool> = DynamicShape.occurrenceKeysOf(name, bindFrom, tree, shape);
		final acc: NarrowAcc = {
			disqualified: false,
			nullUse: false,
			useCount: 0,
	...
	private static function recordWeakSink(
		...
		if (shape.assignKind != null && pk == shape.assignKind && ci == 1) {
			...
			return false;
		if (ty != null && DynamicShape.acceptableType(ty, dynName))
	...
	private static function sourceType(
		...
		if (expr.kind == shape.identKind) {
			...
			return ty != null && DynamicShape.acceptableType(ty, dynName) ? ty : null;
		}
		...
		return nm != null && DynamicShape.acceptableType(nm, dynName) ? nm : null;
	}
