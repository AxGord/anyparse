...
final class AvoidDynamic implements Check implements ConfigAware implements RiskyFix implements OracleAssisted {
	...
	private static function wholeDynamicLocal(
		...
		return wholeDynamicDecl(tree, source, span, shape, dynName, shape.localDeclKinds ?? []);
	}
		...
		for (v in violations) if (v.rule == RULE_ID) {
			...
			switch unifyBagValues(bag.writes, tree, shape, dynName, declaredTypes, imports, oracle, v.file) {
				case Real(t):
					...
					for (op in bag.ops) edits.push({ span: op.call.span ?? span, text: bagOpText(op, source, shape) });
					if (!importAdded) {
						final ie: Null<{ span: Span, text: String }> = importEdit(tree, source);
						if (ie != null) {
							edits.push(ie);
							importAdded = true;
						}
	...
	private static function fixableBagDecl(
		...
		final local: Null<QueryNode> = wholeDynamicDecl(tree, source, span, shape, dynName, ctx.localKinds);
	...
	private static function hasUsingReflect(tree: QueryNode): Bool {
	...
	private static function classifyBagRef(
		...
		final uop: Null<BagOp> = usingBagOp(occ, parent, ci, g, shape, ctx, usingReflect);
		if (uop != null) {
	 ...
	 */
	private static function bagOpText(op: BagOp, source: String, shape: RefShape): String {
	...
	private static function unifyBagValues(
		...
		for (w in writes) {
			var t: Null<String> = bagValueType(w, tree, shape, dynName, declaredTypes);
	...
	private static function bagValueType(
	...
	private static function importEdit(tree: QueryNode, source: String): Null<{ span: Span, text: String }> {
	 ...
	 */
	private static function annotateBags(
		...
		for (v in found) {
			final span: Null<Span> = v.span;
			if (span == null) continue;
			final decl: Null<QueryNode> = wholeDynamicDecl(tree, source, span, shape, dynName, declKinds);
			if (decl == null) continue;
			final bag: Null<BagUses> = bagUsesOf(decl, tree, source, shape, ctx, usingReflect, dynName);
			if (bag == null) continue;
	 ...
	 */
	private static function wholeDynamicDecl(
		tree: QueryNode, source: String, span: Span, shape: RefShape, dynName: String, kinds: Array<String>
	): Null<QueryNode> {
	...
	private static function usingBagOp(
		occ: QueryNode, parent: QueryNode, ci: Int, g: Null<QueryNode>, shape: RefShape, ctx: DynCtx, usingReflect: Bool
	): Null<BagOp> {
