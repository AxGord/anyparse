 ...
 * `Std` and member `string` are matched on node names, not kinds. The outermost matching
 * call is flagged and not descended into, so a nested `Std.string(Std.string(x))` yields
 * one non-overlapping fix. The identifier-safety gate degrades gracefully when `plugin` is
 * not a `TypeInfoProvider` (no declared-type map, treated as empty) — a simple identifier
 * is then never proven safe and is left alone.
 */
@:nullSafety(Strict)
final class PreferInterpolation implements Check {
	...
	public function run(files: Array<{ file: String, source: String }>, plugin: GrammarPlugin): Array<Violation> {
		...
		for (entry in files) {
			...
			walk(violations, entry.file, entry.source, tree, tree, null, false, seams, shape, declaredTypes);
		}
		return violations;
	}
	...
	private static function walk(
		out: Array<Violation>, file: String, source: String, root: QueryNode, node: QueryNode, chain: Null<QueryNode>, inMeta: Bool,
		seams: Seams, shape: RefShape, declaredTypes: Map<Int, String>
	): Void {
		final opaque: Null<Array<String>> = shape.opaqueKinds;
		if (opaque != null && opaque.contains(node.kind)) return;
		// A `Std.string(x)` nested INSIDE a string literal's `${...}` interpolation
		// block must not be rewritten - the result would nest an interpolated
		// string inside an interpolation block, fragile in the real compiler's
		// interp scanner. The walk stops at string literals entirely: their
		// children are interp segments, and the rewrite is not sound in there.
		if ((shape.stringLiteralKinds ?? []).contains(node.kind)) return;
		final arg: Null<QueryNode> = matchArg(node, seams.callKind, seams.fieldAccessKind, seams.identKind);
		if (
			arg != null && !deferredToFold(chain, inMeta, source, seams, shape)
			&& isSafeArg(arg, root, seams.fieldAccessKind, seams.identKind, shape, declaredTypes)
		...
		for (c in node.children) walk(out, file, source, root, c, childChain, childMeta, seams, shape, declaredTypes);
	}
	 ...
	 * nobody. An annotation argument is the same hole spelled differently: the fold
	 * check skips `metaKinds` wholesale, so it owns nothing inside one.
	 *
	 ...
	 * both answer false.
	 */
	private static function deferredToFold(chain: Null<QueryNode>, inMeta: Bool, source: String, seams: Seams, shape: RefShape): Bool {
		final concatKind: Null<String> = seams.concatKind;
		if (inMeta || chain == null || concatKind == null) return false;
