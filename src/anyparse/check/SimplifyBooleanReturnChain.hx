...
final class SimplifyBooleanReturnChain implements Check {
	...
	public function fix(
		...
		// keeps the sound `!(x < 0)` wrap. `run` deliberately builds none — see `reducible`.
	 ...
	 * FLIPS or stays wrapped, never whether the chain reduces at all, so this gate's answer is
	 * resolver-independent — and building one walks the whole resolution scope per file. `fix`
	 * builds it, since only `fix` consumes the text.
	 */
