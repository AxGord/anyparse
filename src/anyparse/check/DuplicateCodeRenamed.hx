 ...
 * layout-insensitive literal-exact render, the non-overlapping occurrence rule, the bare-run
 * filter, the same-file and cross-file passes — and the content gate, measured on what the
 * renaming keeps, so a run that is mostly its binder names can pass the exact reading and not
 * this one. `Info`, REPORT-ONLY: extraction is a refactoring and whether two renamed copies are
 * one idea is a design judgement, so `fix` emits nothing.
 */
@:nullSafety(Strict)
