...
class D {
	...
	public static function flatten(d:Doc):Doc {
		return switch d {
			...
			// structural level).
			case Flatten(inner) | WrapBoundary(inner) | HardFlatten(inner) | CollapseProbe(inner) | CollapseAddProbe(inner) | CollapseBoolProbe(inner) | CollapseChainProbe(inner): flatten(inner);
