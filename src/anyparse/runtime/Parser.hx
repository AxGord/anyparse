...
final class Parser {
	...
	/**
	 * Input keys the schema had no field for, in document order.
	 * Filled by generated ByName parsers whose format declares
	 * `onUnknown = Skip` — the value is still consumed and discarded,
	 * so parse behaviour is unchanged; the record is what lets the
	 * boundary that knows where the input came from say "you wrote a
	 * key I do not know" instead of silently ignoring it.
	 * Empty for every other codepath.
	 *
	 * Append-only, like `errors` and `maxFailPos`: a record made inside a
	 * PEG alternative that later backtracks is NOT unwound. No ByName
	 * struct sits under a backtracking alternative today (every recording
	 * schema is a JSON config whose struct is the root or an array
	 * element), so nothing observes it — a reader that changes that owes
	 * itself a rollback.
	 */
	public final unknownFields: Array<UnknownField> = [];
	...
	/**
	 * Record an input key at `keyPos` that matched no schema field,
	 * carrying the `known` field names so a consumer can suggest the
	 * one that was probably meant. Called from the generated ByName
	 * dispatch's unknown-key arm; never on a success path.
	 */
	public function recordUnknownField(key: String, keyPos: Int, known: Array<String>): Void {
		unknownFields.push(new UnknownField(key, keyPos, known));
	}
