 ...
 * and the owning ctor's `@:trail('}')` consumes the closer that the
 * region's `{` opened.
 *
 ...
 * for.
 *
 * The `}` lives on the CTOR (`@:trail('}')`) rather than on `body`
 * (`@:trail`) so the struct keeps a single Star field and the trail is
 * emitted by the same machinery as `HxDecl.Conditional`'s `#end`.
 *
 ...
 * ordering is safe only because `HxCondBlockOpenRaw` demands a trailing
 * `{` - see that type's doc for why the two ctors are disjoint by
 * construction and why `CondSpliceStmt` would otherwise mis-bind these
 * regions.
 */
@:peg
