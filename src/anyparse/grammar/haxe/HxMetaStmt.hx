 ...
 * self-recursion that a bare try-parse Star would create (parseHxStatement -> MetaStmt -> parseHxStatement ...). `rest` collects further
 * metas; `stmt` recurses into the full statement grammar, so the wrapped statement owns its terminator exactly as it would unprefixed.
 */
@:peg
typedef HxMetaStmt = {
