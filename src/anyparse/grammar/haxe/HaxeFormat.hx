...
final class HaxeFormat implements TextFormat {
	 ...
	 *
	 * `Dynamic` because the callers are generated by the grammar-agnostic
	 * writer lowering, which holds elements as untyped values. Two shapes
	 * arrive: the bare `HxExpr`, and the trivia-synth wrapper
	 * `{node: HxExpr, leadingComments: …}` that a `@:trivia` Star carries.
	 * Anything else — a non-array Star's struct element, a null slot — answers
	 * `false` rather than throwing, so a caller needs no shape gate of its own.
	 */
	public static function isComprehensionGenerator(element: Null<Dynamic>): Bool {
		var node: Null<Dynamic> = element;
		if (node != null && !Reflect.isEnumValue(node)) node = Reflect.field(node, 'node');
		if (node == null || !Reflect.isEnumValue(node)) return false;
		final ctor: String = Type.enumConstructor(node);
		return ctor == 'ForExpr' || ctor == 'WhileExpr';
	}

	/** `complexItemKinds` code for an element that is neither a call/new nor a call-bearing container. */
	public static inline final COMPLEX_ITEM_NONE: Int = 0;

	/** `complexItemKinds` code for an element that IS a function call or a `new` constructor call. */
	public static inline final COMPLEX_ITEM_CALL: Int = 1;

	/** `complexItemKinds` code for a container literal (object / array) carrying a call or `new` in its subtree. */
	public static inline final COMPLEX_ITEM_CONTAINER: Int = 2;

	/**
	 * Depth ceiling of the `complexItemKinds` subtree walk. The Haxe AST is a
	 * finite tree with no back-pointers (`Trivial<T>` carries only the node and
	 * its comment slots), so the cap is not a cycle guard — it bounds the cost
	 * of a pathologically nested container and answers "not complex" past it,
	 * which is the layout-preserving direction.
	 */
	private static inline final COMPLEX_ITEM_MAX_DEPTH: Int = 64;

	/**
	 * Per-element complexity classification of one delimited list, as the
	 * `WrapListOptions.complexItemKinds` axis expects it — one
	 * `COMPLEX_ITEM_*` code per element, length-aligned with the writer's
	 * item Docs.
	 *
	 * "Complex" has ONE definition across both consumers (the
	 * `complexItemCount >= n` wrap condition and the `callParameter` chunk
	 * policy): a `new` expression, a call, or a CONTAINER literal (object or
	 * array) carrying a call / `new` anywhere in its subtree. A container
	 * WITHOUT one — `[{x: 1, y: 2}, {x: 3, y: 4}]` — is NOT complex and keeps
	 * whatever layout width alone gives it.
	 *
	 * The classification is deliberately SEMANTIC. The same `arrayWrap`
	 * cascade also governs array PATTERNS in `case` arms and switch-subject
	 * arrays, whose elements are identifiers, wildcards and literals; a width
	 * proxy (`anyItemLength >= n`) mangles those, while nothing in this
	 * counter can reach them. The one shape that DOES look like a call there
	 * is an enum-constructor pattern (`case [Some(a), Some(b)]`) — measured
	 * over the TM tree before shipping.
	 *
	 * `Dynamic` for the same reason as `isComprehensionGenerator` above: the
	 * callers are generated by the grammar-agnostic writer lowering, which
	 * holds elements untyped, and both the bare node and the trivia-synth
	 * `{node: …}` wrapper arrive here. Anything unrecognised answers
	 * `COMPLEX_ITEM_NONE`.
	 */
	public static function complexItemKinds(elements: Array<Dynamic>): Array<Int> {
		final out: Array<Int> = [];
	...
	/**
	 * True iff `v`'s subtree holds an `HxExpr.Call` or `HxExpr.NewExpr`.
	 * Structure-walk rather than a per-ctor table: the question is "is there a
	 * call ANYWHERE below", so every child of every node has to be visited and
	 * a table would only enumerate the same thing by hand — and go stale the
	 * next time the grammar grows an expression ctor. Enum values recurse
	 * through their parameters, arrays through their elements, anonymous
	 * structures (grammar `typedef`s and the `Trivial<T>` wrapper) through
	 * their fields; leaves answer `false`.
	 */
	private static function subtreeHasCallOrNew(v: Null<Dynamic>, depth: Int): Bool {
		if (v == null || depth > COMPLEX_ITEM_MAX_DEPTH) return false;
