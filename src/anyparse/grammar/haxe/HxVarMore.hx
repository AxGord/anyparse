 ...
 *
 * `@:spanned('VarMore')` makes each continuation an ADDRESSABLE `QueryNode` carrying its own name, so `apq refs` resolves the binding and
 * every declaration-walking check sees it. Without it the projection surfaced only the continuation's INITIALIZER, as a bare extra child
 * of the head declaration: `var a = 0, _b = 0;` read as `(VarStmt a (IntLit 0) (IntLit 0))`, `_b` existed nowhere, and the naming rules
 * were silently blind to it. The kind is published as `RefShape.localDeclContinuationKinds` for consumers enumerating a statement's bound
 * names (`TailMerge.localDeclNames`); the name is lifted off the `decl` field through `QueryWalkerLowering.NAME_UNWRAP_SLOTS`.
 */
@:peg
@:spanned('VarMore')
typedef HxVarMore = {
