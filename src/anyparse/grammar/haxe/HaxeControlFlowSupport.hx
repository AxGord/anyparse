...
/**
 * Haxe's control-flow shape for the `dead-code` check. Blocks are the three
 * statement-sequence nodes: `BlockBody` (a function body), `BlockStmt` (a
 * nested `{ … }` block) and `BlockExpr` (a block used as an expression). A
 * statement unconditionally exits its block when it is `ReturnStmt` (a
 * value return), `VoidReturnStmt` (a bare `return;`) or `ThrowStmt`, or the
 * `break` / `continue` keyword — which parse not as their own kind but as an
 * `ExprStmt` wrapping a bare `IdentExpr` named `break` / `continue`.
 */
@:nullSafety(Strict)
