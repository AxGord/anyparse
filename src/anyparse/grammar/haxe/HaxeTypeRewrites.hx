 ...
 * rewrites consumed by the writer's `@:fmt(preWrite(<fnRef>))` hook on
 * the rule's enum / typedef declaration.
 *
 * Each function takes the full value plus the active `WriteOptions`
 * and returns either `null` (no rewrite, writer falls through to the
 * default emission) or a substitute `HxType` value the writer
 * re-dispatches through its own `writeHxType`. Routing through the
 * same writer means any ctor in the substitute participates in its
 * own `@:fmt(...)` knobs naturally — the rewrite is structural, not
 * output-shaped.
 *
...
final class HaxeTypeRewrites {
	 ...
	 */
	public static function arrowFnOldStyleRewrite(value: HxType, _opt: WriteOptions): Null<HxType> {
		return switch value {
