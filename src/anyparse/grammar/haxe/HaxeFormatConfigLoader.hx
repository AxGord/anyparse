...
/**
 * Loads a haxe-formatter `hxformat.json` config and maps the subset of keys the `HxModule` writer understands
 * onto `HxModuleWriteOptions`. The mapping is strictly additive: a key the loader does not recognise is
 * silently ignored (forward-compatible — the file is haxe-formatter's, hxq models a subset, and refusing would
 * break every real config; `HaxeFormatConfigDiagnostics` reports the unusable ones), and every field it does
 * not find falls back to `HaxeFormat.instance.defaultWriteOptions` — except the eight fork re-baselines
 * (`ifBody` … `doBody`, `caseBody`, `afterLeftCurly`, `beforeRightCurly`) applied at load entry before any
 * section, so `loadHxFormatJson('{}')` formats as the fork does, not as the bare compiled defaults do. The
 * recognised keys are the schema in `HxFormatConfig.hx`; a knob's semantics live on the grammar field carrying
 * the `@:fmt`, or on the knob itself, and `docs/haxe-format-config.md` tabulates every key.
 *
 * Two-stage pipeline: `HxFormatConfigParser` (a macro-generated ByName struct parser) reads the JSON into a
 * typed `HxFormatConfig`; this class maps the struct onto the options — no `JValue` walks, no field-name
 * strings, no state.
 *
 * Vocabulary collapses, applied uniformly wherever a string maps to a policy enum — an unrecognised token
 * keeps the default: `"same"` / `"next"` / `"keep"` → `SameLinePolicy` and `BodyPolicy` (`"fitLine"` → `Same`
 * on a keyword-join site with no `FitLine` branch); `"before"` / `"both"` → `BracePlacement.Next`, `"after"` /
 * `"none"` → `Same` (the inline `{ ... }` shape is not representable); `"before"` / `"onlyBefore"` →
 * `WhitespacePolicy.Before`, `"after"` / `"onlyAfter"` → `After`, `"around"` → `Both`, `"none*"` → `None` (the
 * `only*` / `none*` sides collapse to the four values the writer exposes); a trailing-comma `"yes"` → `true`,
 * anything else → `false` (the AST records no source trailing comma); `WrapRules` sections ingest
 * `defaultWrap` as the cascade's `defaultMode` and `rules` verbatim (`type` → `WrapMode`, `cond` →
 * `WrapConditionType`, an unrecognised `cond` drops the rule; `rules: []` resets to unconditional
 * `defaultMode`); `Int` blank-line counts are override semantics, never a floor.
 *
 * Cascades: each global `lineEnds.leftCurly` / `emptyCurly` / `rightCurly` value is written into its own
 * per-construct knobs, and a per-construct sub-section (`objectLiteralCurly`, `anonFunctionCurly`,
 * `anonTypeCurly`, `blockCurly`, …) wins over the cascade — the fork's `getCurlyPolicy` precedence.
 * `sameLine.expressionIf` fans out on two channels: `keep` / `same` into all three `expression*Body` knobs,
 * `next` / `fitLine` into the if/else pair only (`expressionForBody` has no `else` sibling to fall back
 * through); and the same value ungated into `sameLineExpressionElse`, where `next` maps to `SameOnBlock` —
 * `Same` after a `}` close, `Keep` after any other shape — because the fork's `expressionIf=next` is
 * block-shape-aware. `comments.blockCommentStyle` is anyparse-only: `"verbatim"` / `"javadoc"` /
 * `"javadocNoStars"` reshape MULTI-LINE doc blocks only; `"plain"` is refused because `CommentStyle.Plain`
 * would demote a doc to a plain block.
 */
