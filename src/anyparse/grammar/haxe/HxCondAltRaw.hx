...
/**
 * Raw byte capture of the ALTERNATE branches of a conditional-compilation region: from
 * `#else` / `#elseif` up to AND INCLUDING the closing `#end`.
 *
 * Unlike `HxCondSpliceRaw` — which starts at the `#if` and therefore swallows the FIRST
 * branch as well — this terminal begins at the first alternative, so the region's first
 * branch can be parsed structurally ahead of it. That asymmetry is the point:
 * `HxCondSharedBodyDecl` uses it to keep a type declaration's name, type parameters and
 * heritage in the tree while the parallel headers a preprocessor would have chosen instead
 * survive as bytes:
 *
 * ```haxe
 * #if starling
 ...
 * structurally parseable first branch either, so it never reaches this terminal.
 *
