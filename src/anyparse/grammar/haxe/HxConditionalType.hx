...
/**
 * Body of a `#if <cond> <type>; [#else <type>;] #end` preprocessor-
 * guarded type-position region. Type-scope mirror of
 * `HxConditionalExpr`: the enclosing `HxType.ConditionalType` ctor
 ...
 * single type, and an optional `#else` clause.
 *
 ...
 *
 * `#elseif` chained-clause support is intentionally out of scope —
 * the type-position corpus fixtures use only `#if`/`#else`/`#end`.
 * Adding an `HxElseifType` Star is a separate slice, mirroring how
 * `#elseif` landed for the other scopes (ω-cond-comp-elseif).
 */
@:peg
