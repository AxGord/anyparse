 ...
 * tryparse variants - and, for the enum half, the trivia-mode entries into
 * `TriviaSepLowering` and `TriviaBlockLowering`. The struct half's trivia
 * routes left with `WriterTriviaStarEmitLowering`.
 *
 ...
 * `emitWriterStarField`, and `WriterTriviaStarEmitLowering` its whole trivia
 * route sub-tree (`emitTriviaStar` and the four members only it reaches);
 * neither set is reachable from `lowerEnumStar`, measured on this module's own
 * call graph, so neither fork half is separated from its twin by those splits.
 *
 ...
 */
@:access(anyparse.macro.TriviaBlockLowering, anyparse.macro.TriviaSepLowering, anyparse.macro.WriterCascadeLowering,
	anyparse.macro.WriterChainLowering, anyparse.macro.WriterCondWrapLowering, anyparse.macro.WriterCtorBlankLowering,
	anyparse.macro.WriterCtorPatternLowering, anyparse.macro.WriterFieldSepLowering, anyparse.macro.WriterLowering,
	anyparse.macro.WriterLoweringSupport, anyparse.macro.WriterPolicyLowering, anyparse.macro.WriterRefLeadLowering,
	anyparse.macro.WriterStarPadLowering, anyparse.macro.WriterTriviaStarDispatch, anyparse.macro.WriterTriviaStarEmitLowering)
final class WriterStarEmitLowering {
	...
	private static function emitWriterStarField(
		...
		// `@:tryparse`).
		if (isTriviaStar) {
			WriterTriviaStarEmitLowering.emitTriviaStar(ctx, args, parts);
			return;
		}
