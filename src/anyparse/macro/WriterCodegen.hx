...
import anyparse.macro.WriterOptFanout.*;
 ...
 *
 * What LEFT is the opt-fanout half. `WriterOptFanout` owns which
 * `WriteOptions` context flag a generated `_setXxx` / `_clearXxx` shim
 * sets or clears, the per-grammar gate deciding which of those shims a
 * grammar gets, and the `_copyOpt` / `_b` chain-base mechanism they all
 * share — 48 of this class's 75 members and 1601 of its 2733 lines.
 * Neither half reads state: this class declared no field at all, so the
 * seam is the QUESTION each member answers, not a state boundary. Its
 * members are reached UNQUALIFIED from here through a wildcard import
 * plus the class-level `@:access`, so the split rewrote no call site.
 */
@:access(anyparse.macro.WriterOptFanout)
class WriterCodegen {
