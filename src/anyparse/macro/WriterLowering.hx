 ...
 *
 * The writer lowering is SEVERAL modules. Five carry
 * one responsibility each — `WriterLoweringSupport` (the shared
 * field-access / name / `@:fmt`-argument vocabulary),
 * `WriterPolicyLowering` (the `hxformat.json` policy separators),
 * `WriterCascadeLowering` (the `@:fmt(blankLines*)` cascade),
 * `WriterChainLowering` (`@:fmt(methodChain)`) and
 * `WriterBlankLowering` (the shared source-fidelity probes). Their
 * members are reached UNQUALIFIED from here — a wildcard import plus
 * the class-level `@:access` — which is what let them move without
 * touching a call site, and is why they stayed private.
 *
 * Four more sibling `#if macro` modules of this package carry a trivia
 * Star emit family each — `TriviaTryparseLowering`, `TriviaEofLowering`,
