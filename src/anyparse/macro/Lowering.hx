 ...
 *
 * Nine sibling `#if macro` modules carry one responsibility each out of
 * this pass. Four analyse or name: `ParseDispatchLowering` (the whole
 ...
 *
 * What did NOT leave is what reads instance state. Of the 100 methods this
 * class had, 57 read `_shape`, `_formatInfo`, `_ctx`, `_eregByRule` or
 * `_starGates` directly or through a callee, so moving one costs a signature
 * change at every call site. Five more could have left and did not:
 * `lowerStarNoSepBranch`, `lowerStarSepBranch`, `lowerStarSepAltBranch`,
 * `lowerStarBlockEndedSepStarts` and `lowerStarBlockEndedSepLast` are the
 * shape leaves of the enum-ctor Star path, and that path is the half of the
 * four-site Star+sep audit that pairs with `emitStarFieldSteps`, the
 * struct-field half - both halves stay in one file so the audit stays one
 * read.
 */
@:access(anyparse.macro.BinaryParseLowering, anyparse.macro.KwBranchLowering, anyparse.macro.OperatorLoopLowering,
	anyparse.macro.ParseDispatchLowering, anyparse.macro.SpanArgLowering, anyparse.macro.StarLoopLowering,
	anyparse.macro.StructFieldTrailLowering, anyparse.macro.TriviaSlotNames)
class Lowering {
