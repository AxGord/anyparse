 ...
 *
 * Four sibling `#if macro` modules carry one responsibility each out of
 * this pass: `ParseDispatchLowering` (the whole LL(1) first-token
 * analysis behind the branch guards — `BranchShape`, `BranchFirstToken`
 * and the byte-set algebra they compile to), `BinaryParseLowering` (the
 * `@:binary` field decode emit), `TriviaSlotNames` (the generated
 * parser's trivia-slot local-name vocabulary) and `PrattMeta` (the
 * operator annotations, shared with `WriterLowering`, which used to
 * carry its own copy). Their members are reached UNQUALIFIED from here
 * through a wildcard import plus the class-level `@:access`, so the
 * split rewrote no call site.
 */
@:access(anyparse.macro.BinaryParseLowering, anyparse.macro.ParseDispatchLowering, anyparse.macro.TriviaSlotNames)
class Lowering {
