...
class TriviaAnalysis {
	...
	private static function markStarsWithTrivia(node: ShapeNode): Void {
		...
		if (node.kind == Star && hasTrivia(node.annotations[AnnotationKeys.BASE_META])) {
			node.annotations[AnnotationKeys.TRIVIA_STAR_COLLECTS] = true;
		}
		...
		if (node.kind == Seq && hasTrivia(node.annotations[AnnotationKeys.BASE_META])) {
	...
	private static function markPostfixStarSuffix(node: ShapeNode): Void {
		...
		if (
			node.kind == Seq && hasPostfixPair(node.annotations[AnnotationKeys.BASE_META]) && node.children.length == 2
	...
	private static function collectRefsInto(node: ShapeNode, out: Array<String>): Void {
		if (node.kind == Ref) {
			final r: Null<String> = node.annotations[AnnotationKeys.BASE_REF];
