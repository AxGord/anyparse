...
final class CliWalk {
	...
	/**
	 * The "did you mean" candidates for `query` over the keys of `pool` — `EditDistance.closest`
	 * with a set spelled as a map, which is how the three CLI call sites already hold their
	 * vocabularies. The ranking and its two tiers live with the metric: an engine-side caller
	 * (a `--select` segment kind against the grammar's projected set) cannot reach this package.
	 */
	public static function findFuzzy(query: String, pool: Map<String, Bool>): Array<String> {
		return EditDistance.closest(query, [for (cand in pool.keys()) cand]);
	}
