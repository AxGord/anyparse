...
final class MutationVerdictCommand implements CliCommand {
	...
	private static function runMutationVerdict(args: Array<String>): Int {
		...
		// reading with two `read -r` calls sees no behaviour change, and one
		// reading a third line only has to check it is non-empty.
		if (verdict.full != null) CliIo.sysPrint('${verdict.full}\n');
		return EXIT_OK;
	}
