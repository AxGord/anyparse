...
| `possible-null-dereference` | a dereference of a provably-nullable expression with no null check, a possible NPE; `Info`, report-only. Slice 1 of the reference-nullable family (mechanism B — the type-driven, point-wise sibling of the flow-sensitive `null-dereference`). Three nullable sources: a `Map`-family index `m[k]` (a `Null<V>`), an `Array` / `List` `pop` / `shift` call (a `Null<T>`), and a call to a function whose declared return type is `Null<T>` — covering `m[k].field` / `m[k].method()` / `m[k]!.field`, `arr.pop().field` / `arr.shift().method()`, and `findUser().field`. Type-aware and it must be: `m[k].field` shares an identical AST with `arr[i].field`, `arr.pop().field` with a `pop()` call on any type, and `findUser().field` with any call; only the receiver's declared type / the callee's declared return type tells them apart — a `Map` index yields `Null<V>` (an `Array` / `String` index a non-null `T`), `Array.pop` / `Array.shift` / `List.pop` yield `Null<T>` (a same-named method on an unrelated type does not), and a `Null<T>`-returning function yields a nullable result. So the deref flags only when the receiver is a `RefShape.nullableIndexTypeNames` index (`Map` / `StringMap` / `IntMap` / `ObjectMap` / `EnumValueMap` / `WeakMap`) or a `RefShape.nullableInstanceReturnCalls` (`Array.pop` / `Array.shift` / `List.pop`) call on a plain identifier of matching declared type (`TypeResolver.identTypeName`), or a call whose plain-identifier callee binds to a function whose `TypeInfoProvider.returnTypes` outer nominal is a `RefShape.nullableReturnMarkerTypes` (`Null`); all resolution requires `plugin is TypeInfoProvider`. An `Array` / `String` / unannotated / `Null<Map<…>>` receiver, an unrelated-type method, a non-`Null<…>` or unannotated return, and a qualified `this.f()` / `obj.f()` callee are safe misses. Point-wise, NOT flow-sensitive — `if (m.exists(k)) m[k].field`, `if (arr.length > 0) arr.pop().f` and a guarded `findUser().f` are still flagged (the guard is invisible without flow), which is why the severity is `Info` (advisory), not the `Warning` a flow-sensitive engine would earn. A bare `m[k]` / `arr.pop()` / `findUser()` with no deref, `m.get(k)` (a `Call`, not `[]`), and a cross-file return are left alone; macro-reification subtrees are not descended into |
