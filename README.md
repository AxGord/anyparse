...
| `possible-null-dereference` | a dereference of a provably-nullable expression with no null check, a possible NPE; `Info`, report-only. Slice 1 of the reference-nullable family (mechanism B — the type-driven, point-wise sibling of the flow-sensitive `null-dereference`). Two nullable sources: a `Map`-family index `m[k]` (a `Null<V>`) and an `Array` / `List` `pop` / `shift` call (a `Null<T>`) — covering `m[k].field` / `m[k].method()` / `m[k]!.field` and `arr.pop().field` / `arr.shift().method()` / `lst.pop()!.field`. Type-aware and it must be: `m[k].field` shares an identical AST with `arr[i].field`, and `arr.pop().field` with a `pop()` call on any type; only the receiver's declared type tells them apart — a `Map` index yields `Null<V>` (an `Array` / `String` index a non-null `T`), and `Array.pop` / `Array.shift` / `List.pop` yield `Null<T>` (a same-named method on an unrelated type does not). So the deref flags only when the receiver is a plain identifier whose declared outer nominal type (`TypeResolver.identTypeName`, requiring `plugin is TypeInfoProvider`) is one of `RefShape.nullableIndexTypeNames` (the `Map` family: `Map` / `StringMap` / `IntMap` / `ObjectMap` / `EnumValueMap` / `WeakMap`) for an index, or matches a `RefShape.nullableInstanceReturnCalls` (`Array.pop` / `Array.shift` / `List.pop`) `Type.method` for a call; an `Array` / `String` / unannotated / `Null<Map<…>>` (outer name `Null`) / unrelated-type receiver is a safe miss. Point-wise, NOT flow-sensitive — `if (m.exists(k)) m[k].field` and `if (arr.length > 0) arr.pop().f` are still flagged (the guard is invisible without flow), which is why the severity is `Info` (advisory), not the `Warning` a flow-sensitive engine would earn. A bare `m[k]` / `arr.pop()` with no deref, `m.get(k)` (a `Call`, not `[]`), and a `Null<T>`-returning free function are left alone; macro-reification subtrees are not descended into |
