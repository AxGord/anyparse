...
| `possible-null-dereference` | a dereference of a `map[key]` result — a `Null<V>` — with no null check, a possible NPE; `Info`, report-only. Slice 1 of the reference-nullable family (mechanism B — the type-driven, point-wise sibling of the flow-sensitive `null-dereference`). Type-aware and it must be: `m[k].field`, `m[k].method()` (its callee `FieldAccess`) and `m[k]!.field` share an identical AST with `arr[i].field`, and only the index receiver's declared type tells them apart — a `Map` index yields `Null<V>`, an `Array` / `String` index a non-null `T`. So the deref flags only when the `IndexAccess` receiver is a plain identifier whose declared outer nominal type (`TypeResolver.identTypeName`, requiring `plugin is TypeInfoProvider`) is one of `RefShape.nullableIndexTypeNames` (the `Map` family: `Map` / `StringMap` / `IntMap` / `ObjectMap` / `EnumValueMap` / `WeakMap`); an `Array` / `String` / unannotated / `Null<Map<…>>` (outer name `Null`) receiver is a safe miss. Point-wise, NOT flow-sensitive — `if (m.exists(k)) m[k].field` and `m[k] = v; m[k].field` are still flagged (the guard is invisible without flow), which is why the severity is `Info` (advisory), not the `Warning` a flow-sensitive engine would earn. A bare `m[k]` with no deref, and `m.get(k)` (a `Call`, not `[]`), are left alone; macro-reification subtrees are not descended into |
