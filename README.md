...
| `prefer-enum-abstract` | a class / abstract declaring three or more `static [inline] final <PREFIX>_*` numeric constants that share a name prefix (`RANK_ACCESSOR` / `RANK_CONSTRUCTOR` → `RANK`) — they read as a closed enumeration but carry no distinct type, so any `Int` flows where one is meant; `Info`, report-only, suggesting an `enum abstract`. The prefix is the relatedness signal (a `MAX` / `TIMEOUT` pair is not a set); one finding per prefix group. Left alone: an existing enum abstract, a mutable `static var`, an instance `final`, a non-numeric constant, a prefix-less name, and a sub-threshold (< 3) group. Report-only because the enum-abstract flavour depends on usage the check does not analyse — ordinal `<` needs `@:op(A < B)`, bit-flags need `from Int to Int`, pure `==` / `switch` needs neither |
