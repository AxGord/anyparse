...
| `comparison-to-boolean` | a comparison against a boolean literal (`x == true` / `x != false`) where the literal adds nothing; `Info`, report-only — an operand whose nullness cannot be ruled out without types is skipped: a `?.` access, a call / `Map.get` result, a possibly-`@:optional` field (`obj?.flag == true`, `map.get(k) == true`, `obj.flag == true`), since that `== true` is load-bearing on a `Null<Bool>` under strict null-safety; comparisons inside macro reification are skipped too. Only a bare identifier or boolean-operator operand is reported |
