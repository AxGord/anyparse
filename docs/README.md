...
| [`strategies.md`](strategies.md) | reference | 30 KB | `anyparse.core.Strategy` (the member contract lives on the interface), `anyparse.macro.strategy.*` (each tag's argument forms live on the class), `unit.lowering.FmtFlagOwnershipTest` (reads the `@:fmt` inventory block from this file) | the strategy plugin picture: which tags each strategy owns and where its slots are consumed, the `@:fmt` inventory and ownership table, what a macro-module split has to pass |
