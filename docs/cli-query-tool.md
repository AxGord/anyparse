...
**What each import rule does with a chain.** `unused-import` asks the governance seam who the READERS of a statement are: an ordinary module reads its own, an ambient source's readers are the modules it governs — judged against its own text every ambient statement reads as dead and the fix deletes a load-bearing import. Because the readers are read from disk, a run narrowed to the ambient source alone answers the same as a whole-tree one; a module read that way arrives with no extracted import list, which only makes the used-test more generous, the direction a deleting verdict must err in. `redundant-import` owns the opposite case — a statement the chain already puts in force — under ONE positive criterion: an unguarded `import` / `using` whose simple name the NEAREST ambient binder binds through an unguarded statement of the very same kind and path, on a bounded chain, with nothing else in the file binding that name. An identical statement binds an identical thing whatever it names, which is why that arm consults no declaration; a nearer ambient source binding the name to something ELSE is what makes the file's own statement load-bearing, and it is refused. It is `RiskyFix` like the rest of the rule, so the deletion needs a `compilerOracle`. `duplicate-import` sees a file's own statements only, so an own statement equal to an ambient one is never a duplicate — it is `redundant-import`'s finding, and repetition WITHIN an ambient source is `duplicate-import`'s, exactly as in any other module. `import-order` and `fmt` treat an ambient source as the module it is: a `using` opens its own block there too, and the round trip is idempotent.
