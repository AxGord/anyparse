...
`@:sep(";", tailRelax, blockEnded("<predicate>"))` is the option (b2) AST-shape variant. In addition to the byte-check `}` / `;`, the Star primitive calls `schema.instance.<predicate>(<accum>[<accum>.length - 1])` on the just-pushed element to decide sep-elision by AST shape. The predicate is a schema-method on the plugin's HaxeFormat-shaped class, reached through the same channel as `trailOptParseGate` and `unescapeChar`. Required to cover ident-terminated stmts (e.g. `x is String` — `HxStatement.ExprStmt(Is)`) and `]`-terminated stmts (`[1, 2, 3]` — `HxStatement.ExprStmt(ArrayExpr)`) which the byte-check cannot cover safely (`]` would silently accept `arr[0] foo()`). The predicate is wired by the helper `Lowering.buildBlockEndedPredicateCall`. The grammar-side AST predicate for Haxe statements lives in `HxExprUtil.stmtNoSemi`, exposed via `HaxeFormat.stmtNoSemi`. See project memory `project_blockbody_star_session7_b2_infra` for the migration roadmap.
