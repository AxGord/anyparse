...
- Next recon-confirmed sole-blocker clusters: no-brace expression-bodied function *declaration* (`function f() <expr>` at member scope, the real gap behind `sameline/issue_83_*` ×4 — distinct from the anon fn-*expr* no-brace body, which already parses); `#error` directive (`indentation/issue_298_*`, ×3); `{> Base, fields}` structure-inheritance typedef (`whitespace/issue_202_*`, ×2). (`.$ident` field-access closed by Slice 4.)
