...
- Next recon-confirmed sole-blocker clusters: `#error` directive (`indentation/issue_298_*`, ×3 — but inside `#if cs … #end`, so a strip to empty `#if #end` is itself rejected → must strip `#error '…'` to a dummy decl/stmt to confirm sole-blocker, expect compounding with the empty-cond-comp-body limitation); `{> Base, fields}` structure-inheritance typedef (`whitespace/issue_202_*`, ×2, plus a `{ > Foo, var foo; }` variant in the histogram). (`.$ident` field-access closed by Slice 4; no-brace fn-decl body by Slice 5.)
