...
- 🔶 Haxe formatter CLI that takes a `.hx` file and outputs formatted Haxe. **Shipping today as js/node** (`bin/apq.js`, `hxq fmt`). A native binary stays open on hxcpp; **neko is ruled out** — the neko build of the CLI compiles but its artifact dies at module load on every invocation (`Stack check failed for function scope`), measured 2026-08-17 when `bin/apq.hxml` was deleted.
...
**Slice-by-slice history** (what landed, per slice): moved to
