# CLI query tool journal

> **Journal, not contract.** Every number here is a reading of one tree at one moment; the
> contract lives in [`docs/cli-query-tool.md`](../cli-query-tool.md). Each block is the ORIGINAL text of a paragraph
...
## From § Command surface › `apq search`
...
## From § Command surface › `--doc` / `--source` (opt-in, on `refs` / `uses` / `ast`)
...
## From § Command surface › `apq resolve-define`
...
## From § Command surface › Input path forms
...
## From § Command surface › The cost of a ROUND: batched queries, the TTY progress gate, and the whole-file read guard
...
## From § Command surface › Parse-failure locus
...
## From § Command surface › `apq probe`: the staged scratch slot resolves per PROCESS (T700)
...
## From § Choosing between `patch` and `replace-node`, and what NOT to type (S197)
...
## From § `--fix` on a write op: the lint pass, scoped to the lines the write changed
...
**A pointer is not a record.** A marker inside a path or a URL, and one on a doc-tag line (`@see`, `@link`), point AT a record instead of carrying one — which is exactly what the rule asks prose to leave behind. And a string literal is never visited at all: the seam is the comment scan, so a hash or a duration inside a fixture's source is data.

**The percent sign is a unit, except where it is an operator.** A number followed by the sign is a share, read the way a duration's unit is read. The same sign written with a number on its RIGHT is the remainder operator, and prose about operator precedence writes several of them on one line — so no share is taken from a line that shows one. The guard is per LINE rather than per sign, which is what settles the nested spelling `20 % (12 % 7)`: there the sign a share would be read from has a parenthesis on its right, and only the inner one carries a digit.

**The recording verb is read by POSITION, not by case.** Opening a clause — as the comment's own first word, right after an opening parenthesis, or separated by a space from the punctuation that closed the clause before it — the verb introduces a claim and needs no number beside it. Mid-clause the same word is a project's ordinary vocabulary for how a width or a region is obtained (`is measured by`, `the measured width`, `a Doc-measured predicate`, `flat-measurement walks`, an appositive after a comma); that states a contract, and stays silent unless a number stands on its line. Position rather than capitalisation, because prose wraps: a sentence continued onto the next comment line begins that line mid-clause, and this project writes its emphasis in upper case.

**One finding per comment, and it says how many readings the comment holds.** The finding is anchored at the first, because the reader is being asked to rewrite the block either way; the count the message carries (`(3 readings)`) is what separates a line to delete from a block to rewrite. Readings that OVERLAP are one — a chain of arrows records a single progression, so `0 -> 2 -> 4 -> 6` counts once rather than once per arrow.
...
## From § `apq rewrite`: a template is a TREE, so it is spliced as one
...
## From § `apq fmt`: canonicalise source through the writer (T718)
...
## From § Pattern syntax for `search` (frozen for v1)
...
## From § Pattern syntax for `search` (frozen for v1) › The `...` ellipsis
...
## From § Pattern syntax for `search` (frozen for v1) › Non-features in v1
...
## From § Selector syntax for `ast --select` (v2)
...
## From § Selector syntax for `ast --select` (v2) › The kind vocabulary is checked
...
## From § Selector syntax for `ast --select` (v2) › Non-features
...
## From § Op addressing: `--select` / `--match` / `--nth` / positions
...
## From § Op addressing: `--select` / `--match` / `--nth` / positions › A module-level declaration's RAW span reaches the next one — the ops do not
...
## From § Op addressing: `--select` / `--match` / `--nth` / positions › What a removal reports, and why it is a report rather than a refusal
...
## From § Output formats › Output JSON schemas (v1, finalized)
...
## From § Output formats › Output JSON schemas (v1, finalized) › `search`
...
## From § Output formats › Output JSON schemas (v1, finalized) › `refs`
...
## From § Output formats › Output JSON schemas (v1, finalized) › `meta`
...
## From § Output formats › Kind vocabulary
...
## From § Universalization invariant
...
## From § Resolved decisions (Phase 0)
...
## From § Resolved decisions (Phase 0) › Metavariable reuse: structural-identity unification
...
## From § Resolved decisions (Phase 0) › Star-children matching: ordered and adjacent by default
...
## From § Resolved decisions (Phase 0) › Whitespace and comments in patterns: both ignored
...
## From § Open questions deferred to later phases
