# CLI query tool — Phase 5 dogfood history (archived)

Friction log and completed per-slice logs for cli-query-roadmap.md Phase 5.
Moved out of the main roadmap; current status lives in docs/cli-query-roadmap.md.
...
- **Slice K3 — multi-value `case A, B, C:` patterns. ✅ DONE.**
  ...
  **254/280 → 256/280** (newly passing `HxExprUtil.hx` — the file
  ...
  per user directive #2 — "neko is too slow"; js = real codegen
...
- **Slice L — closing the 24-fail tail (NOT additive; core forks).**
  ...
  - **Slice X1 — `$ident` in the var/final name slot. ✅ DONE
    ...
    **#2** an asymmetric `if (c) bareExpr else { block }` at statement
    ...
    before implementation). User chose to push through the
