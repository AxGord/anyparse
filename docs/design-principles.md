...
**What does NOT hold — and the reason this principle was rewritten**: the runtime *around* the generated code keeps process-scoped caches, and they make a parallel parse inside one process unsafe. The offenders are named rather than described: `SharedParseTier` (its static vars), `FormatConfigDiscovery.CACHE`, `HaxeQueryPlugin.extMethodsCache`, and the one-entry root memo the query walker generates (`_memoSource` / `_memoRoot`). Evidence: a multi-threaded JVM parse lost about half its nodes with parse failures and **zero exceptions** — silent corruption, not a crash — while hxcpp segfaults at four threads or more (the readings are in `docs/journal/design-principles-log.md`). Handing each thread its own plugin instance does **not** help, because the state is static, not per-instance.
...
**Consequence**: parallelism is **processes**, not threads. That is a deliberate position, not a defeat — process fan-out reaches roughly 3x with bit-exact output, with the knee at four processes (two independent readings, in the journal). Note what that is *not*: the earlier claim here read "an 8-core machine gets 8x throughput, not 1x". It does not. Anything that keeps per-run state must therefore be **run-scoped** — an instance field on a wrapper created per lint run or fix run — and adding another process-scoped cache is a regression against this principle even when it is faster.
...
## 6. Writing is `format(ast, options)`; what the parser keeps is the grammar's decision

**Rule**: the writer regenerates output from AST + FormatOptions; nothing patches the source text. What the parser keeps is a grammar decision, not a writer one: the plain parser drops whitespace, comments and stylistic choices (quote styles, trailing commas), and the trivia-mode parser records comments and the blank-line / newline shape as data on the AST (`anyparse.runtime.Trivial`) for the `keep` policies to read — the same single writer pass either way.
...
**Consequence**: "I parsed this file, modified one line, and want to write it back with minimal diff" is handled by the trivia-mode parse plus writer options that match the original file's style (`keep` policies, the project's own `hxformat.json`); the canonical-output claim holds for whatever the options do not ask to keep. Byte-identity through the writer (`writeRoundTrip(s) == s`) is the gate the writer-emit ops run, so a file the writer would restyle is refused rather than silently reformatted.
