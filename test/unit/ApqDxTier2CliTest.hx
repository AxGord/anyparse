...
class ApqDxTier2CliTest extends Test {
	...
	public function testSweepReadsTotals():Void {
		#if sys
		final snapshot:String = CliFixture.writeAs('apq_sweep', 'json',
			'{"pass":42,"fail":7,"skipParse":3,"skipWrite":0,"skipConfig":1,"skipMalformed":1}');
		Assert.equals(0, Cli.run(['sweep', '--file', snapshot]),
			'reading a valid snapshot exits 0');
	...
	public function testSweepMissingFileExitsRuntime():Void {
		#if sys
		Assert.equals(1, Cli.run(['sweep', '--file', '/tmp/definitely_does_not_exist.json']),
			'missing file → EXIT_RUNTIME');
		#else
		Assert.pass('non-sys target');
		#end
	}
	...
	public function testSweepDeltaVsPrev():Void {
		#if sys
		final cur:String = CliFixture.writeAs('apq_sweep_cur', 'json',
			'{"pass":533,"fail":267,"skipParse":98,"skipWrite":0,"skipConfig":1,"skipMalformed":1}');
		final prev:String = CliFixture.writeAs('apq_sweep_prev', 'json',
			'{"pass":533,"fail":264,"skipParse":101,"skipWrite":0,"skipConfig":1,"skipMalformed":1}');
		Assert.equals(0, Cli.run(['sweep', '--file', cur, '--prev', prev]),
			'delta-vs-prev exits 0 on read success');
	...
	public function testSweepAcceptsInjectedLangFlag():Void {
		...
		final snapshot:String = CliFixture.writeAs('apq_sweep', 'json',
			'{"pass":1,"fail":0,"skipParse":0,"skipWrite":0,"skipConfig":0,"skipMalformed":0}');
	...
	public function testCasesFindsCallPattern():Void {
		...
		Assert.equals(0, Cli.run(['cases', 'VarMember', f]),
			'case VarMember(d): → exit 0 with hits');
	...
	public function testCasesFindsBareIdentPattern():Void {
		...
		final f:String = CliFixture.write('apq_cases', src);
		Assert.equals(0, Cli.run(['cases', 'FinalMember', f]),
			'case FinalMember: (bare ident) → exit 0');
		FileSystem.deleteFile(f);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testCasesSkipsImports():Void {
		...
		final f:String = CliFixture.write('apq_cases', src);
		Assert.equals(0, Cli.run(['cases', 'VarMember', f]),
			'no case-patterns → exit 0 (empty walker, not error)');
		FileSystem.deleteFile(f);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testCasesMissingArgsExitsUsage():Void {
		...
		Assert.equals(2, Cli.run(['cases', 'VarMember']),
			'one arg → EXIT_USAGE (missing path)');
	}
	...
	public function testAstChildrenLimitClampsHorizontalWidth():Void {
		...
		Assert.equals(0, Cli.run(['ast', f, '--depth', '1', '--children-limit', '2']),
			'--children-limit 2 should run cleanly on a 5-child root');
		FileSystem.deleteFile(f);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testAstChildrenLimitRejectsNegative():Void {
		...
		Assert.equals(2, Cli.run(['ast', f, '--children-limit', '-1']),
			'negative integer → EXIT_USAGE');
		FileSystem.deleteFile(f);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testAstChildrenLimitRejectsNonInt():Void {
		#if sys
		final f:String = CliFixture.write('apq_ast_climit', 'class C {}');
		Assert.equals(2, Cli.run(['ast', f, '--children-limit', 'foo']),
			'non-int value → EXIT_USAGE');
		FileSystem.deleteFile(f);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testLitAutoWidensOnDefault0Hit():Void {
		...
		Assert.equals(0, Cli.run(['lit', 'HxSharpErrorSliceTest', f]),
			'auto-widen exits 0 with hits, not 0-with-nudge');
		FileSystem.deleteFile(f);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testLitExplicitKindDoesNotAutoWiden():Void {
		...
		final src:String = 'import unit.HxSharpErrorSliceTest;\n';
		final f:String = CliFixture.write('apq_lit_widen', src);
		Assert.equals(0, Cli.run(['lit', 'HxSharpErrorSliceTest', f, '--kind', 'Literal']),
