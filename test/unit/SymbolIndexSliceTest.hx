...
class SymbolIndexSliceTest extends Test {
	...
	/**
	 * STRUCTURAL membership: whether a container satisfies `Iterable` / `Iterator`, and the
	 * static-extension accept gate that consumes it. Membership is decided by the members the
	 * type declares — `iterator()` for an `Iterable`, `hasNext()` + `next()` for an `Iterator` —
	 * never by an `implements` clause, which no stdlib container carries.
	 */
	public function testStructuralIterableMembership(): Void {
		final index: SymbolIndex = SymbolIndex.build(iterableFiles(), plugin());
	...
	/** The structural-membership fixture set: one iterable pair, five near-misses, and a generic extension module. */
		...
		return [
			...
			{ file: 'DeepBag.hx', source: 'class DeepBag {\n\tpublic function iterator():deep.DeepIter return null;\n}' },
			{
				...
				source: 'class IterExt {\n\tpublic static function pick<T>(it:Iterable<T>):Widget return null;\n\n'
					...
					+ '\tpublic static function nest<T>(it:Iterable<Iterable<T>>):Widget return null;\n\n'
