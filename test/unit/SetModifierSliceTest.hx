...
class SetModifierSliceTest extends Test {
	...
	public function testFlipVisibility(): Void {
		final text: String = okText(SetModifier.setModifier(PRIVATE_FN, 3, 1, ['public'], true, new HaxeQueryPlugin()));
	...
	public function testFlipAndAddStatic(): Void {
		final text: String = okText(SetModifier.setModifier(PRIVATE_FN, 3, 1, ['public', '+static'], true, new HaxeQueryPlugin()));
	...
	public function testRemoveInline(): Void {
		...
		final text: String = okText(SetModifier.setModifier(src, 3, 1, ['-inline'], true, new HaxeQueryPlugin()));
		Assert.isTrue(text.contains('public function f'));
	...
	public function testAddVisibilityToBare(): Void {
		...
		final text: String = okText(SetModifier.setModifier(src, 3, 1, ['public'], true, new HaxeQueryPlugin()));
		Assert.isTrue(text.contains('public function f'));
	}

	/** A `final` change is rejected (it wraps the declaration). */
	public function testFinalChangeRejected(): Void {
		Assert.isTrue(isErr(SetModifier.setModifier(PRIVATE_FN, 3, 1, ['+final'], true, new HaxeQueryPlugin())));
	}

	/** An unknown modifier is rejected. */
	public function testUnknownRejected(): Void {
		Assert.isTrue(isErr(SetModifier.setModifier(PRIVATE_FN, 3, 1, ['+frobnicate'], true, new HaxeQueryPlugin())));
	}
