...
class ApqAstChildCountTest extends Test {
	...
	private static inline final FIXTURE:String =
		'enum X {\n'
		+ '	Zero;\n'
		+ '	One(a:Int);\n'
		+ '	Two(a:Int, b:Int);\n'
	...
	public function testMinChildrenKeepsMultiArgOnly():Void {
		...
		Assert.equals(0, Cli.run(['ast', '--select', 'ParamCtor', '--min-children', '2', fixture]),
	...
	public function testMaxChildrenKeepsSingleArgOnly():Void {
		#if sys
		final fixture:String = writeFixture(FIXTURE);
		Assert.equals(0, Cli.run(['ast', '--select', 'ParamCtor', '--max-children', '1', fixture]),
	...
	public function testBoundsAreInclusive():Void {
		...
		Assert.equals(0, Cli.run(['ast', '--select', 'ParamCtor', '--min-children', '2', '--max-children', '2', fixture]),
	...
	public function testEmptyResultStaysCleanExit():Void {
		...
		Assert.equals(0, Cli.run(['ast', '--select', 'ParamCtor', '--min-children', '10', fixture]),
	...
	public function testRejectsNegativeValue():Void {
		#if sys
		final fixture:String = writeFixture(FIXTURE);
		Assert.equals(2, Cli.run(['ast', '--select', 'ParamCtor', '--min-children', '-1', fixture]),
			'--min-children rejects negative integers');
		Assert.equals(2, Cli.run(['ast', '--select', 'ParamCtor', '--max-children', 'nope', fixture]),
			'--max-children rejects non-integer values');
		FileSystem.deleteFile(fixture);
		#else
		Assert.pass('non-sys target');
		#end
