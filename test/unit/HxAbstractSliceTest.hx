...
class HxAbstractSliceTest extends HxTestHelpers {
	...
	public function testSingleFrom(): Void {
		...
		switch ad.clauses[0] {
			case FromClause(type):
				Assert.equals('Int', (expectNamedType(type).name: String));
			case _:
				Assert.fail('expected FromClause');
		}
	}
	...
	public function testSingleTo(): Void {
		...
		switch ad.clauses[0] {
			case ToClause(type):
				Assert.equals('String', (expectNamedType(type).name: String));
			case _:
				Assert.fail('expected ToClause');
		}
	}
	...
	public function testMultipleFromTo(): Void {
		...
		switch ad.clauses[0] {
			case FromClause(type):
				Assert.equals('Int', (expectNamedType(type).name: String));
			case _:
				Assert.fail('expected FromClause at 0');
		}
		switch ad.clauses[1] {
			case FromClause(type):
				Assert.equals('Float', (expectNamedType(type).name: String));
			case _:
				Assert.fail('expected FromClause at 1');
		}
		switch ad.clauses[2] {
			case ToClause(type):
				Assert.equals('String', (expectNamedType(type).name: String));
			case _:
				Assert.fail('expected ToClause at 2');
		}
		switch ad.clauses[3] {
			case ToClause(type):
				Assert.equals('Bool', (expectNamedType(type).name: String));
			case _:
				Assert.fail('expected ToClause at 3');
		}
	}
