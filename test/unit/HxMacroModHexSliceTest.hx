...
class HxMacroModHexSliceTest extends HxTestHelpers {
	...
	public function testBareZeroStillIntLit():Void {
		...
		switch decl.init {
			...
			case null, _: Assert.fail('expected IntLit(0), got ${decl.init}');
		}
	}
	...
	public function testFloatStillFloatLit():Void {
		...
		switch decl.init {
			case FloatLit(_): Assert.pass();
			case null, _: Assert.fail('expected FloatLit, got ${decl.init}');
		}
	}
	...
	public function testMacroModifierBare():Void {
		...
		switch m.member {
			case FnMember(decl): Assert.equals('f', (decl.name : String));
	...
	public function testMacroModifierBuildHxShape():Void {
		...
		final ast:HxClassDecl = HaxeParser.parse(
			'class Foo { public static macro function buildParser():Void {} }'
		);
		final m:HxMemberDecl = ast.members[0];
		...
		switch m.member {
			case FnMember(decl): Assert.equals('buildParser', (decl.name : String));
			case _: Assert.fail('expected FnMember');
		}
	}
