...
class HxSwitchBareSliceTest extends HxTestHelpers {
	...
	public function testSwitchStmtBareCallSubject(): Void {
		...
		switch sw.expr {
			...
			case null, _:
				Assert.fail('expected Call subject');
		}
		Assert.equals(1, sw.cases.length);
	}
	...
	public function testSwitchStmtBareFieldAccessSubject(): Void {
		...
		switch sw.expr {
			...
			case null, _:
				Assert.fail('expected FieldAccess subject');
		}
	}
	...
	public function testSwitchStmtBareIsSubject(): Void {
		...
		switch sw.expr {
			case Is(_, _):
				Assert.pass();
			case null, _:
				Assert.fail('expected Is subject');
		}
	}
	...
	public function testSwitchExprBareInReturn(): Void {
		final body: Array<HxStatement> = parseBody('class C { function f():String { return switch x { case 1: "a"; case _: "b"; }; } }');
	...
	public function testSwitchExprBareInVarInit(): Void {
		final body: Array<HxStatement> =
			parseBody('class C { function f():Void { var y:String = switch x { case 1: "a"; case _: "b"; }; } }');
		Assert.equals(1, body.length);
		switch body[0] {
			case VarStmt(decl):
				switch decl.init {
	...
	public function testSwitchWithParensStillRoutesToSwitchStmt(): Void {
		...
		switch body[0] {
			...
			case SwitchStmtBare(_):
				Assert.fail('parens form must route to SwitchStmt, not SwitchStmtBare');
			case null, _:
