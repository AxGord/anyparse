 ...
 * `sameLine.functionBody: @:default(Next)` to preserve pre-slice
 * byte-output across the corpus; the default flip is a separate
 * audit.
 */
@:nullSafety(Strict)
class HxFunctionBodyPolicySliceTest extends Test {
	...
	public function testDefaultIsSame():Void {
		...
		Assert.equals(BodyPolicy.Same, defaults.functionBody);
	}
