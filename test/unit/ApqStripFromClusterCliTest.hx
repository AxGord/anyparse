...
class ApqStripFromClusterCliTest extends Test {
	...
	public function testFromClusterRequiresCorpusRoot():Void {
		...
		Assert.equals(1, Cli.run(['strip', '--from-cluster', 'X', '--delete', 'foo']),
	...
	public function testFromClusterMissingDirIsRuntimeError():Void {
		#if sys
		Assert.equals(1, Cli.run(['strip', '--from-cluster', 'X', '/no/such/dir/xyz123', '--delete', 'foo']),
	...
	public function testFromClusterRequiresSubstitution():Void {
		...
		Assert.equals(2, Cli.run(['strip', '--from-cluster', 'X', dir]),
			'--from-cluster without --replace/--with or --delete is a usage error');
		cleanupDir(dir);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testFromClusterMultiplePositionalsIsUsageError():Void {
		#if sys
		Assert.equals(2, Cli.run(['strip', '--from-cluster', 'X', '/a', '/b', '--delete', 'foo']),
	...
	public function testFromClusterEmptyCorpusIsCleanExit():Void {
		...
		Assert.equals(1, Cli.run(['strip', '--from-cluster', 'anything', dir, '--delete', 'foo']),
	...
	public function testFromClusterUnknownKeyIsRuntimeError():Void {
		...
		Assert.equals(1, Cli.run(['strip', '--from-cluster', 'xyz-not-a-real-key', dir, '--delete', 'foo']),
