...
class HxConditionalSourceBlankSliceTest extends Test {
	...
	public function testDefaultDropsSourceBlankBeforeConditional(): Void {
		...
		Assert.isFalse(
			out.indexOf('B;\n\n#if') >= 0, 'default knob=false: head-transparency + betweenImports=0 drops the source blank before #if'
		);
		Assert.isTrue(out.indexOf('B;\n#if') >= 0, 'tight emit between import and #if when knob is off');
	}

	public function testOptInPreservesSourceBlankBeforeConditional(): Void {
		final out: String = writeWith('import a.B;\n\n#if sys\nimport sys.FileSystem;\n#end', true);
		Assert.isTrue(out.indexOf('B;\n\n#if') >= 0, 'knob=true: source blank between import and #if survives the transparency override');
	}

	public function testOptInDoesNotInsertBlankWhenSourceHasNone(): Void {
		final out: String = writeWith('import a.B;\n#if sys\nimport sys.FileSystem;\n#end', true);
		Assert.isFalse(out.indexOf('B;\n\n#if') >= 0, 'knob=true is preservation-only — does NOT synthesise a blank where source had none');
	}
