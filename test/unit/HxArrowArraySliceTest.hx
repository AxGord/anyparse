...
class HxArrowArraySliceTest extends HxTestHelpers {
	...
	public function testSingleIdentLambda(): Void {
		...
		switch decl.init {
			...
			case null, _:
				Assert.fail('expected Arrow(IdentExpr, Add), got ${decl.init}');
		}
	}
	...
	public function testArrowRightAssoc(): Void {
		...
		switch decl.init {
			...
			case null, _:
				Assert.fail('expected Arrow(x, Arrow(y, Add)), got ${decl.init}');
		}
	}
	...
	public function testAssignVsArrow(): Void {
		...
		switch decl.init {
			...
			case null, _:
				Assert.fail('expected Assign(a, Arrow(b, c)), got ${decl.init}');
		}
	}
	...
	public function testArrowInVarInit(): Void {
		...
		switch decl.init {
			...
			case null, _:
				Assert.fail('expected Arrow(x, x), got ${decl.init}');
		}
	}
	...
	public function testArrowInReturn(): Void {
		...
		switch stmts[0] {
			case ReturnStmt(Arrow(IdentExpr(l), IdentExpr(r))):
				...
				Assert.equals('x', (r: String));
			case null, _:
				Assert.fail('expected ReturnStmt(Arrow)');
		}
	}
	...
	public function testParenExprNotLambda(): Void {
		...
		switch decl.init {
			case ParenExpr(Add(_, _)):
				Assert.pass();
			case null, _:
				Assert.fail('expected ParenExpr(Add), got ${decl.init}');
		}
	}
	...
	public function testArrayThenIndex(): Void {
		...
		switch decl.init {
			...
			case null, _:
				Assert.fail('expected IndexAccess(ArrayExpr, IntLit), got ${decl.init}');
		}
	}
		...
		switch decl.init {
			case ArrayExpr(elems):
				...
				switch elems[0] {
					...
					case null, _:
						Assert.fail('expected Arrow in first entry');
				}
				switch elems[1] {
					case Arrow(IntLit(k), DoubleStringExpr(_)):
	...
	public function testArrowInModule(): Void {
		...
		switch va.init {
			case Arrow(_, _):
				Assert.pass();
			case null, _:
				Assert.fail('expected Arrow');
		}
		switch vb.init {
			case ArrayExpr(elems):
				Assert.equals(2, elems.length);
			case null, _:
				Assert.fail('expected ArrayExpr');
		}
	}
	...
	public function testWordBoundaryArrowed(): Void {
		...
		switch decl.init {
			case IdentExpr(v):
				Assert.equals('arrowed', (v: String));
			case null, _:
