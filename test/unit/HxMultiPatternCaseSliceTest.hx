...
class HxMultiPatternCaseSliceTest extends HxTestHelpers {
	...
	public function testSinglePatternIsOneElementList(): Void {
		...
		switch b.patterns[0].expr {
			case Plain(IntLit(v)):
				Assert.equals(1, v);
			case null, _:
				Assert.fail('expected Plain(IntLit) pattern, got ${b.patterns[0].expr}');
		}
	}
	...
	public function testTwoPatternCase(): Void {
		...
		switch b.patterns[0].expr {
			case Plain(IntLit(v)):
				Assert.equals(1, v);
			case null, _:
				Assert.fail('expected Plain(IntLit 1)');
		}
		switch b.patterns[1].expr {
			case Plain(IntLit(v)):
				Assert.equals(2, v);
			case null, _:
				Assert.fail('expected Plain(IntLit 2)');
		}
	}

	public function testThreePatternStringCase(): Void {
		final sw: HxSwitchStmt = parseSwitch(
			'class C { function f(s:String):Void { switch (s) { case "a", "b", "c": y(); case _: z(); } } }'
		);
		final b: HxCaseBranch = caseBranch(sw.cases[0]);
	...
	public function testMultiPatternWithBody(): Void {
		final sw: HxSwitchStmt = parseSwitch(
			'class C { function f(x:Int):Void { switch (x) { case 1, 2, 3: { a(); b(); } case _: z(); } } }'
		);
		final b: HxCaseBranch = caseBranch(sw.cases[0]);
		Assert.equals(3, b.patterns.length);
	...
	public function testMixedMultiSingleAndDefault(): Void {
		final sw: HxSwitchStmt = parseSwitch(
			'class C { function f(x:Int):Void { switch (x) { case 1, 2: a(); case 3: b(); default: c(); } } }'
		);
		...
		switch sw.cases[2] {
			case DefaultBranch(_):
				Assert.pass();
			case null, _:
				Assert.fail('expected DefaultBranch, got ${sw.cases[2]}');
		}
	}
