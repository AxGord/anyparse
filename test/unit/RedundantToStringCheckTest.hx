...
class RedundantToStringCheckTest extends Test {
	...
	public function testConcatAbstractReceiverReportedNotFixed(): Void {
		...
		final src: String = DECL_ABSTRACT
			+ "@:nullSafety(Strict) class C { function f(s:String, t:Tag) { final r:String = s + t.toString(); } }";
		final found: Array<Violation> = violations(src);
		Assert.equals(1, found.length);
	...
	public function testChainedCallNotFlagged(): Void {
		...
		Assert.equals(
			0,
			violations(
				DECL_K + "@:nullSafety(Strict) class C { function f(s:String, x:K) { final r:String = s + x.toString().substr(0); } }"
			).length
		);
	}
	...
	public function testCommentInRemovedTextSuppressesFix(): Void {
		final src: String = DECL_K
			+ "@:nullSafety(Strict) class C { function f(s:String, x:K) { final r:String = s + x /* keep */ .toString(); } }";
		Assert.equals(1, violations(src).length);
		Assert.equals(0, edits(src).length);
	}
