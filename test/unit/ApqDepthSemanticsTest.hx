...
class ApqDepthSemanticsTest extends Test {
	...
	public function testDepthZeroOnModule():Void {
		...
		Assert.equals(0, Cli.run(['ast', '--code', 'class C { var x:Int = 0; }', '--depth', '0']),
			'--depth 0 without --select is a clean exit');
	}

	public function testDepthOnePlusOnModule():Void {
		Assert.equals(0, Cli.run(['ast', '--code', 'class C { var x:Int = 0; }', '--depth', '1']),
			'--depth 1 (module + one child level) is a clean exit');
		Assert.equals(0, Cli.run(['ast', '--code', 'class C { var x:Int = 0; }', '--depth', '5']),
			'--depth 5 (full tree) is a clean exit');
	}
	...
	public function testDepthZeroWithSelect():Void {
		...
		Assert.equals(0, Cli.run(['ast', '--code', 'class C { var x:Int = 0; }', '--select', 'VarMember', '--depth', '0']),
	...
	public function testDepthOneWithSelect():Void {
		...
		Assert.equals(0, Cli.run(['ast', '--code', 'class C { var x:Int = 0; }', '--select', 'VarMember', '--depth', '1']),
	...
	public function testDepthZeroWithAt():Void {
		...
		Assert.equals(0, Cli.run(['ast', '--code', 'class C { var x:Int = 0; }', '--at', '1:15', '--depth', '0']),
	...
	public function testDepthNegativeIsFullTree():Void {
		...
		Assert.equals(0, Cli.run(['ast', '--code', 'class C { var x:Int = 0; }', '--depth', '-1']),
			'negative --depth is a clean exit (full tree)');
	}

	public function testDepthNonIntegerIsUsageError():Void {
		Assert.equals(2, Cli.run(['ast', '--code', 'class C {}', '--depth', 'nope']),
			'non-integer --depth is a usage error');
	}
}
