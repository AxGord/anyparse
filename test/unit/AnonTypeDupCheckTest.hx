...
class AnonTypeDupCheckTest extends Test {
	...
	public function testDifferentFieldTypesDoNotGroup(): Void {
		...
		Assert.isTrue(six[0].message.contains('xml:Xml'), six[0].message);
		Assert.isTrue(six[1].message.contains('xml:Int'), six[1].message);
	}
	...
	public function testFieldOrderIsNormalised(): Void {
		...
		Assert.isTrue(vs[0].message.contains('{ text:String, xml:Xml }'), 'the key renders sorted: ${vs[0].message}');
	}
	...
	public function testOptionalFieldIsPartOfTheKey(): Void {
		...
		Assert.equals(1, vs.length);
		Assert.isTrue(vs[0].message.contains('?xml:Xml'), vs[0].message);
	}
	...
	public function testNestedStructureIsPartOfTheKey(): Void {
		...
		Assert.equals(1, vs.length);
		Assert.isTrue(vs[0].message.contains('p:{ q:Int }'), vs[0].message);
	}
	...
	public function testTypeArgumentsArePartOfTheKey(): Void {
		...
		Assert.equals(1, vs.length);
		Assert.isTrue(vs[0].message.contains('m:Map<A, B>'), vs[0].message);
	}
	...
	public function testRegisteredInBuiltinsAndOffByDefault(): Void {
		Assert.equals(169, Linter.builtins().length);
