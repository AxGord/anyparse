...
final class HxIfStmtExprPositionDispatchTest extends Test {
	...
	public function testElseBodyDualFlagDispatchesAtExpressionPosition(): Void {
		...
		final out: String = HaxeModuleTriviaWriter.write(HaxeModuleTriviaParser.parse(src), opts);
		Assert.isTrue(
			out.indexOf('if (c) a(); else b();') != -1, 'expressionElseBody=Same must flatten else-body at expression-position: <$out>'
		);
	}
	...
	public function testReturnSingleLineValueUsesSingleLineFlag(): Void {
		...
		Assert.isTrue(
			outSame.indexOf('return 42;') != -1, 'single-line value follows returnBodySingleLine=Same (not returnBody=Next): <$outSame>'
		);
