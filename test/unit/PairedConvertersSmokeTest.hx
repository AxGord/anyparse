...
class PairedConvertersSmokeTest extends utest.Test {
	...
	public function testRawToPairedRoundTripDeclCount(): Void {
		...
		final paired2: anyparse.grammar.haxe.trivia.Pairs.HxModuleT = anyparse.grammar.haxe.trivia.Pairs.Converters.rawToPaired_HxModule(
			raw1
		);
	...
	public function testRawToPairedRoundTripMembers(): Void {
		final paired1: anyparse.grammar.haxe.trivia.Pairs.HxModuleT = HaxeModuleTriviaParser.parse(
			'class Foo { var x:Int; function bar():Void {} }'
		);
		final raw1: HxModule = anyparse.grammar.haxe.trivia.Pairs.Converters.pairedToRaw_HxModule(paired1);
		final paired2: anyparse.grammar.haxe.trivia.Pairs.HxModuleT = anyparse.grammar.haxe.trivia.Pairs.Converters.rawToPaired_HxModule(
