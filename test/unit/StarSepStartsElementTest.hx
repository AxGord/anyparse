...
class StarSepStartsElementTest extends Test {
	...
	public function testParseSingleEmptyAtom():Void {
		...
		switch ast {
			case Block([EmptyAtom]): Assert.pass();
			case _: Assert.fail('expected Block([EmptyAtom]), got $ast');
		}
	}
	...
	public function testParseTwoEmptyAtoms():Void {
		...
		final ast:MiniBlockStrict = MiniBlockStrictParser.parse('{;;}');
		switch ast {
			case Block([EmptyAtom, EmptyAtom]): Assert.pass();
			case _: Assert.fail('expected Block([EmptyAtom, EmptyAtom]), got $ast');
		}
	}
	...
	public function testParseThreeEmptyAtoms():Void {
		final ast:MiniBlockStrict = MiniBlockStrictParser.parse('{;;;}');
		switch ast {
			case Block([EmptyAtom, EmptyAtom, EmptyAtom]): Assert.pass();
			case _: Assert.fail('expected Block of 3 EmptyAtoms, got $ast');
		}
	}
