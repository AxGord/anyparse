...
class ApqDxTier4CliTest extends Test {
	...
	public function testAstCountWithSelectExitsClean():Void {
		Assert.equals(0, Cli.run([
			'probe', 'class M { var a:Int; var b:Int; }', '--select', 'ClassDecl', '--count'
		]),
	...
	public function testGatesHelpExitsClean():Void {
		Assert.equals(0, Cli.run(['gates', '--help']),
			'apq gates --help is a clean exit');
	}

	public function testGatesUnknownOptionIsUsageError():Void {
		Assert.equals(2, Cli.run(['gates', '--bogus']),
			'apq gates unknown option is a usage error');
	}
	...
	public function testGatesOnEmptyDirEmitsEmpty():Void {
		...
		Assert.isTrue(exit == 0 || exit == 1,
			'apq gates on empty dir returns 0 (empty walk) or 1 (no inputs matched), got $exit');
		cleanupDir(dir);
		#else
	...
	public function testPredictRelaxRejectsReplaceWith():Void {
		Assert.equals(2, Cli.run([
			'recon', '--predict-relax', '--replace', 'x', '--with', 'y', '/some/dir'
		]),
	...
	public function testPredictRelaxIncompatibleWithPredictStrip():Void {
		Assert.equals(2, Cli.run([
			'recon', '--predict-relax', '--predict-strip', '--replace', 'x', '--with', 'y', '/some/dir'
		]),
	...
	public function testTopLevelUsageExitsClean():Void {
		...
		Assert.equals(0, Cli.run(['--help']),
			'top-level --help exits clean');
	}
