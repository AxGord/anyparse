...
class HxIncrDecrSliceTest extends HxTestHelpers {
	...
	public function testPostIncrIdent():Void {
		...
		switch decl.init {
			case PostIncr(IdentExpr(v)): Assert.equals('a', (v : String));
			case null, _:
	...
	public function testPostDecrIdent():Void {
		final decl:HxVarDecl = parseSingleVarDecl('class Foo { var x:Int = a--; }');
		switch decl.init {
			case PostDecr(IdentExpr(v)): Assert.equals('a', (v : String));
			case null, _:
	...
	public function testPreIncrIdent():Void {
		final decl:HxVarDecl = parseSingleVarDecl('class Foo { var x:Int = ++a; }');
		switch decl.init {
			case PreIncr(IdentExpr(v)): Assert.equals('a', (v : String));
			case null, _:
	...
	public function testPreDecrIdent():Void {
		final decl:HxVarDecl = parseSingleVarDecl('class Foo { var x:Int = --a; }');
		switch decl.init {
			case PreDecr(IdentExpr(v)): Assert.equals('a', (v : String));
			case null, _:
