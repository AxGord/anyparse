 ...
 * consumed run held a newline and no comment (`omega-untyped-keep`) — so the rewind fires at
 * THIS site over this project's own sources, and across all FOUR members that splice this
 * same block — see
 * `unit.lowering.StarBlockEndedWsRewindSitesTest` for the other three.
 *
 * Most of this site's fires cannot change the answer: the byte the rewind lands on is `}`
 * or a comment's last character, which is not `;` either way. The rest land ON a `;`, and
 * there the rewind is the whole difference between `_isBE` and a thrown
