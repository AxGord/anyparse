...
class ApqHxqDxV10CliTest extends Test {
	...
	public function testCasesUnwrapsPlainWrapper():Void {
		...
		final fixture:String = CliFixture.write('apq_cases_v10',
			'class C { function f(s:Dynamic) { switch s { case VarStmt(_): trace(""); case _: } } }');
	...
	public function testCasesNoMatchSilent():Void {
		...
		#if sys
		final fixture:String = CliFixture.write('apq_cases_v10',
			'class C { function f() { switch x { case 1: trace(""); case _: } } }');
		Assert.equals(0, Cli.run(['cases', 'VarStmt', fixture]));
		FileSystem.deleteFile(fixture);
		#else
		Assert.pass('non-sys target');
	...
	public function testCasesAlternationStillMatches():Void {
		...
		#if sys
		final fixture:String = CliFixture.write('apq_cases_v10',
			'class C { function f(s:Dynamic) { switch s { case Foo | VarStmt: trace(""); case _: } } }');
		Assert.equals(0, Cli.run(['cases', 'VarStmt', fixture]));
		FileSystem.deleteFile(fixture);
		#else
		Assert.pass('non-sys target');
	...
	public function testWriterProbeRunsCleanOnSourcePreservingFixture():Void {
		...
		#if sys
		final fixture:String = CliFixture.write('apq_wp_v10',
			'class M {\n\tfunction m() {\n\t\tvar a = 1;\n\t}\n}\n');
	...
	public function testSweepDiffEmitsPerFixtureFlips():Void {
		#if sys
		final cur:String = CliFixture.writeAs('apq_sweep_v10_cur', 'json', '{'
			+ '"pass":2,"fail":1,"skipParse":0,'
			+ '"fixtures":['
			...
			+ ']}');
		final prev:String = CliFixture.writeAs('apq_sweep_v10_prev', 'json', '{'
			+ '"pass":1,"fail":1,"skipParse":1,'
			+ '"fixtures":['
	...
	public function testSweepDiffIdenticalSnapshotsEmitsZeroChanged():Void {
		#if sys
		final cur:String = CliFixture.writeAs('apq_sweep_v10_id', 'json', '{'
			+ '"pass":1,"fail":0,"skipParse":0,'
			+ '"fixtures":[{"path":"a","status":"PASS"}]}');
		final prev:String = CliFixture.writeAs('apq_sweep_v10_id', 'json', '{'
			+ '"pass":1,"fail":0,"skipParse":0,'
			+ '"fixtures":[{"path":"a","status":"PASS"}]}');
		Assert.equals(0, Cli.run(['sweep', '--file', cur, '--diff', prev]));
		FileSystem.deleteFile(cur);
	...
	public function testSweepDiffComposesWithPrev():Void {
		...
		#if sys
		final cur:String = CliFixture.writeAs('apq_sweep_v10_compose', 'json', '{'
			+ '"pass":2,"fail":0,"skipParse":0,'
			+ '"fixtures":[{"path":"a","status":"PASS"},{"path":"b","status":"PASS"}]}');
		final prev:String = CliFixture.writeAs('apq_sweep_v10_compose', 'json', '{'
			+ '"pass":1,"fail":1,"skipParse":0,'
			+ '"fixtures":[{"path":"a","status":"PASS"},{"path":"b","status":"FAIL"}]}');
	...
	public function testSweepDiffMissingFixturesArrayFails():Void {
		#if sys
		final cur:String = CliFixture.writeAs('apq_sweep_v10_nof', 'json',
			'{"pass":1,"fail":0,"skipParse":0}');
		final prev:String = CliFixture.writeAs('apq_sweep_v10_nof', 'json',
			'{"pass":0,"fail":1,"skipParse":0,"fixtures":[{"path":"a","status":"FAIL"}]}');
	...
	public function testSearchPlainMetavarStillWorks():Void {
		...
		// them as patterns.
		#if sys
		final fixture:String = CliFixture.write('apq_search_v10',
			'class C { function f() { trace(1); } }');
