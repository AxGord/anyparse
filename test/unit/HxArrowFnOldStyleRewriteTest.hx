...
class HxArrowFnOldStyleRewriteTest extends HxTestHelpers {
	...
	public function testRewriteSinglePositionalArrow(): Void {
		...
		Assert.notNull(out);
		switch out {
			case Arrow(Parens(Arrow(_, _)), Named(_)):
				Assert.pass();
			case _:
				Assert.fail('expected Arrow(Parens(Arrow), Named), got $out');
		}
	}
	...
	public function testWriterEmitsTightOldStyleChain(): Void {
		...
		Assert.isTrue(out.indexOf('var f:(Int->Int)->Int->Int;') != -1, 'expected tight `(Int->Int)->Int->Int`, got: <$out>');
	}
	...
	public function testWriterEmitsTightCurriedChain(): Void {
		...
		Assert.isTrue(out.indexOf('var f:(R->Void)->(E<X>->Void)->Void;') != -1, 'expected fully tight chain, got: <$out>');
	}
	...
	public function testWriterPreservesAroundSpacedNewStyle(): Void {
		...
		Assert.isTrue(out.indexOf('var f:(Int) -> Int;') != -1, 'expected around-spaced `(Int) -> Int`, got: <$out>');
	}
	...
	public function testWriterPreservesAroundSpacedNamedArg(): Void {
		...
		Assert.isTrue(out.indexOf('var f:(name:String) -> Void;') != -1, 'expected `(name:String) -> Void`, got: <$out>');
	}
	...
	public function testWriterPreservesAroundSpacedMultiArg(): Void {
		...
		Assert.isTrue(out.indexOf('var f:(Int, String) -> Bool;') != -1, 'expected `(Int, String) -> Bool`, got: <$out>');
	}

	public function testWriterPreservesEmptyParens(): Void {
		final out: String = writeDefault('class Foo { var f:() -> Void; }');
		Assert.isTrue(out.indexOf('var f:() -> Void;') != -1, 'expected `() -> Void`, got: <$out>');
	}
