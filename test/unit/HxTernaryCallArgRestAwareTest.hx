...
final class HxTernaryCallArgRestAwareTest extends Test {
	...
	public function testTernaryCallArgWrapsWhenCommaOverflows(): Void {
		Assert.equals(
			...
			+ '\t\t\tmeasureB >= (LAYOUT_SECOND_SPAN * zoomFactor) ? LAYOUT_SECOND_SPAN / 2 : (measureB / 2 - scrollAnchor.y) '
			+ '* pixelsPerContentUnitX\n\t\t);\n\t}\n\n}',
			triviaWrite(
				'class Sample {\n\tfunction run() {\n\t\tviewport.setTo(measureA >= (LAYOUT_PRIMARY_SPAN * zoomFactor) '
				+ '? LAYOUT_PRIMARY_SPAN / 2 : (measureA / 2 - scrollAnchor.x) * pixelsPerContentUni, measureB >= (LAYOUT_SECOND_SPAN * '
				+ 'zoomFactor) ? LAYOUT_SECOND_SPAN / 2 : (measureB / 2 - scrollAnchor.y) * pixelsPerContentUnitX);\n\t}\n}',
				CFG
			)
		);
	}
	...
	public function testTernaryCallArgFlatAtExactLimit(): Void {
		Assert.equals(
			'class Sample {\n\n\tfunction run() {\n\t\tviewport.setTo(\n\t\t\tmeasureA >= (LAYOUT_PRIMARY_SPAN * zoomFactor) '
			+ '? LAYOUT_PRIMARY_SPAN / 2 : (measureA / 2 - scrollAnchor.x) * pixelsPerContentUn,\n\t\t\tmeasureB >= (LAYOUT_SECOND_SPAN * '
			+ 'zoomFactor) ? LAYOUT_SECOND_SPAN / 2 : (measureB / 2 - scrollAnchor.y) * pixelsPerContentUnitX\n\t\t);\n\t}\n\n}',
			triviaWrite(
				'class Sample {\n\tfunction run() {\n\t\tviewport.setTo(measureA >= (LAYOUT_PRIMARY_SPAN * zoomFactor) '
				+ '? LAYOUT_PRIMARY_SPAN / 2 : (measureA / 2 - scrollAnchor.x) * pixelsPerContentUn, measureB >= (LAYOUT_SECOND_SPAN * '
				+ 'zoomFactor) ? LAYOUT_SECOND_SPAN / 2 : (measureB / 2 - scrollAnchor.y) * pixelsPerContentUnitX);\n\t}\n}',
				CFG
			)
		);
