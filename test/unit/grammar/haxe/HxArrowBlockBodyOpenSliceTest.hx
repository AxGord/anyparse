...
final class HxArrowBlockBodyOpenSliceTest extends Test {
	...
	/**
	 * DISCRIMINATES: a chain whose LAST link carries a wide expression-bodied arrow keeps the block-bodied
	 * `.onFinished` link cuddled (`-> {`); without the gate the rest-of-stack lookahead strands the `{` on its own line.
	 */
	...
	/**
	 * CONTROL: the same chain with a NARROW trailing-link argument -- the rest-of-stack
	 * sum stays under `maxLineLength`, so the arrow was already cuddled before the gate.
	 */
	...
	/**
	 * CONTROL (boundary, fits side): trailing-link argument one char SHORTER than the sibling below -- the chain cascade breaks the
	 * chain one link per line first, so the arrow marker never surfaces and the rendering is byte-identical with the gate reverted.
	 */
	...
	/**
	 * DISCRIMINATES (boundary, fits+1 side): ONE more char in the trailing-link argument re-glues the chain
	 * and hands the decision to the arrow marker -- without the gate that is exactly where `-> \n {` appears.
	 */
	...
	/**
	 * DISCRIMINATES: real-tree shape A (anonymized) -- a `put(...).success(block arrow).error(expression
	 * arrow)` crash-report chain whose last link was collapsed to an expression body.
	 */
	...
	/**
	 * DISCRIMINATES: real-tree shape B (anonymized) -- a tween `.onFinished(block
	 * arrow).onTicked(expression arrow)` chain inside a property setter.
	 */
	...
	/**
	 * DISCRIMINATES: shape B again with `wrapping.methodChainCuddledLinks` ON
	 * -- the gate composes with the cuddled-links knob, same cuddled rendering.
	 */
	...
	/**
	 * CONTROL (narrowness pin): an EXPRESSION-bodied sole arrow arg still breaks after `->` with its close `)`
	 * on its own line -- the gate keys on a `{`-leading body only and did not disable the marker wholesale.
	 */
	...
	/**
	 * DISCRIMINATES the guard's SECOND conjunct (`hasForcedBreak`): a FLAT object-literal arrow body also starts with
	 * `{`, but it rides the head line in full, so breaking after `->` genuinely shortens that line and must still
	 * happen. A `{`-only guard suppressed the break and pushed this fixture from 127 to 141 columns against the 140
	 * budget. Reverting the whole arm leaves this test PASSING -- it pins the guard's width, not its existence.
	 */
	...
	/**
	 * CONTROL (outcome pin, byte-identical with the arm reverted AND with its second conjunct removed): an object literal whose OWN wrap
	 * cascade already committed to breaking hardlines right after `{`, so it terminates the head line itself and keeps `{` cuddled --
	 * exactly as a statement block does. It pins the slice OUTCOME for the second, non-block half of the gated population, not the gate.
	 */
	...
	private inline function triviaWrite(src: String, ?cfg: String): String {
		return HxWriteFixture.triviaWrite(src, cfg ?? CFG);
	}
