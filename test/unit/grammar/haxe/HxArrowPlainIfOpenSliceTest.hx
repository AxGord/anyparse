...
final class HxArrowPlainIfOpenSliceTest extends Test {
	...
	public function testSingleArgOverflowingPlainIfOpensCall(): Void {
		final src: String = 'class M {\n\tfunction singleArg() {\n\t\tcallThing((v:Null<PayloadRecordType>) -> if (v != null) '
			+ 'registry.applyPendingBatch((rec:SessionEntryType) -> rec.metaTag.fieldEntry = v));\n\t}\n}';
		Assert.equals(
			'class M {\n\tfunction singleArg() {\n\t\tcallThing(\n\t\t\t(v:Null<PayloadRecordType>) -> if (v != null) '
			+ 'registry.applyPendingBatch((rec:SessionEntryType) -> rec.metaTag.fieldEntry = v)\n\t\t);\n\t}\n}',
			triviaWrite(src)
		);
	}
	...
	public function testTwoArgOverflowingPlainIfOpensCall(): Void {
		final src: String = 'class M {\n\tfunction twoArg() {\n\t\tpost({}, (v:Null<PayloadRecordType>) -> if (v != null) '
			+ 'registry.applyPendingBatch((rec:SessionEntryType) -> rec.metaTag.fieldEntry = v));\n\t}\n}';
		Assert.equals(
			'class M {\n\tfunction twoArg() {\n\t\tpost(\n\t\t\t{},\n\t\t\t(v:Null<PayloadRecordType>) -> if (v != null) '
			+ 'registry.applyPendingBatch((rec:SessionEntryType) -> rec.metaTag.fieldEntry = v)\n\t\t);\n\t}\n}',
