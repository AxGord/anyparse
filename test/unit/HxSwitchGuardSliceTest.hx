...
class HxSwitchGuardSliceTest extends HxTestHelpers {
	...
	public function testCaseWithGuardPresent():Void {
		...
		switch p.guard {
			...
			case e: Assert.fail('expected guard ParenExpr, got $e');
		}
	}
	...
	public function testGuardRoundTrip():Void {
		roundTrip(
			'class C { function f(x:E):Void { switch (x) { case A if (b): y(); case C, D if (e): z(); case _: w(); } } }',
			'switch-guard'
		);
	}
}
