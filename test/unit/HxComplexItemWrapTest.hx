 ...
 * `anyItemLength >= n` rule reproduces the same two targets and then ALSO explodes `case [A, B]`
 * patterns and switch-subject arrays, which a semantic counter cannot reach — except through the
 * one shape that looks like a call in pattern position, which `PATTERNS_SRC` below pins.
 *
 ...
 * before and after packed around it. In FIRST-argument position the same container is HUGGED by the
 * head (`new Stack([` … `], w, 46)`); that hug is emergent engine behaviour, so the D1 fixture
 * asserts the broken array and the surviving hug in ONE string — asserting only the broken array
 * would still pass if the slice had opened the hug into a ladder.
 *
...
final class HxComplexItemWrapTest extends Test {
	...
	/** A call whose LAST argument is an array holding one call-bearing object literal. */
	 ...
	 * gate existed; this fixture is what goes red if `@:fmt(suppressComplexItems)` is removed from
	 * `HxCasePattern.expr` / `HxSwitchStmt(Bare).expr`. It does NOT discriminate against the
	 * pre-slice writer — nothing counted anything there — which is exactly why it is worth keeping.
	 */
	 ...
	 * open paren. This asserts the glue still declines for a non-first container.
	 */
