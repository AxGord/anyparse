...
class HxParamBodyUntypedSliceTest extends HxTestHelpers {
	...
	public function testUntypedParamWithDefault():Void {
		...
		switch body.defaultValue {
			...
			case null, _: Assert.fail('expected IntLit default, got ${body.defaultValue}');
		}
	}
	...
	public function testUntypedExprBodyNoReturnType():Void {
		...
		Assert.isNull(body.type);
		switch decl.body {
			case ExprBody(ReturnExpr(IdentExpr(name))): Assert.equals('x', (name : String));
			case _: Assert.fail('expected ExprBody(ReturnExpr(IdentExpr)), got ${decl.body}');
		}
	}
