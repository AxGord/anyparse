...
final class HxObjectLitWrapRulesTest extends Test {
	...
	public function testThreeFieldOverflowingLineWraps(): Void {
		...
		final src: String = 'class Foo { static var x = {'
			+ 'alpha: "first really really really really really long value here", beta: "second really really really really '
			+ 'really long value here", gamma: "third really really really really really long value here"}; }';
		final out: String = writeWith(src);
	...
	public function testCustomNoWrapDefaultDisablesEngine(): Void {
		...
		Assert.isTrue(
			out.indexOf('{one: 1, two: 2, three: 3, four: 4, five: 5}') != -1, 'expected forced flat layout under empty rule set: <$out>'
		);
	}
	...
	private static inline function decide(
		rules: WrapRules, itemCount: Int, maxItemLen: Int, totalItemLen: Int, exceedsMaxLineLength: Bool
	): WrapMode {
		return WrapList.decideWithLineLengthState(rules, itemCount, maxItemLen, totalItemLen, exceedsMaxLineLength, false, t -> false);
	}
