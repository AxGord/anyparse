...
class HxBodySliceTest extends HxTestHelpers {
				...
				switch expr {
					case Call(operand, args):
						switch operand {
							...
							case null, _:
								Assert.fail('expected IdentExpr(foo)');
						}
				...
				switch expr {
					case Call(operand, args):
						switch operand {
							...
							case null, _:
								Assert.fail('expected FieldAccess');
						}
						Assert.equals(0, args.length);
					case null, _: Assert.fail('expected Call');
				}
				...
				switch expr {
					case Assign(left, right):
						switch left {
							...
							case null, _:
								Assert.fail('expected IdentExpr(x)');
						}
						switch right {
							case IntLit(v): Assert.equals(1, (v: Int));
							case null, _: Assert.fail('expected IntLit(1)');
	...
	public function testMultipleExprStmts(): Void {
		...
		Assert.equals(3, stmts.length);
		switch stmts[0] {
			case ExprStmt(IntLit(v)):
				Assert.equals(1, (v: Int));
			case null, _:
				Assert.fail('expected ExprStmt(IntLit(1))');
		}
		switch stmts[1] {
			case ExprStmt(IntLit(v)):
				Assert.equals(2, (v: Int));
			case null, _:
				Assert.fail('expected ExprStmt(IntLit(2))');
		}
		switch stmts[2] {
			case ExprStmt(IntLit(v)):
				Assert.equals(3, (v: Int));
			case null, _:
				Assert.fail('expected ExprStmt(IntLit(3))');
		}
	}
	...
	public function testBodyThroughModuleRoot(): Void {
		...
		switch aStmts[0] {
			case ReturnStmt(IntLit(v)):
				Assert.equals(1, (v: Int));
			case null, _:
				Assert.fail('expected ReturnStmt(IntLit(1))');
		}
		...
		switch bStmts[0] {
			case ExprStmt(IdentExpr(v)):
				Assert.equals('x', (v: String));
			case null, _:
				Assert.fail('expected ExprStmt(IdentExpr(x))');
		}
	}
