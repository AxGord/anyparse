...
class HxSwitchNewSliceTest extends HxTestHelpers {
			...
			case CaseBranch(branch):
				...
				switch branch.patterns[0].expr {
					case Plain(IntLit(v)):
						Assert.equals(1, v);
					case null, _:
						Assert.fail('expected Plain(IntLit) pattern');
				}
	...
	public function testMultipleCases(): Void {
		...
		switch sw.cases[0] {
			...
			case null, _:
				Assert.fail('expected first CaseBranch');
		}
		switch sw.cases[1] {
			case CaseBranch(b):
				Assert.equals(1, b.body.length);
			case null, _:
				Assert.fail('expected second CaseBranch');
		}
	}
	...
	public function testDefaultBranch(): Void {
		...
		switch sw.cases[0] {
			case DefaultBranch(branch):
				Assert.equals(1, branch.stmts.length);
			case null, _:
				Assert.fail('expected DefaultBranch');
		}
	}
			...
			case CaseBranch(b):
				switch b.patterns[0].expr {
					case Plain(IntLit(v)):
						Assert.equals(1, v);
					case null, _:
						Assert.fail('expected Plain(IntLit)');
				}
				Assert.equals(1, b.body.length);
			case null, _:
				Assert.fail('expected CaseBranch');
	...
	public function testNestedSwitch(): Void {
		final sw: HxSwitchStmt = parseSwitch('class C { function f():Void { switch (x) { case 1: switch (y) { case 2: z; } } } }');
		Assert.equals(1, sw.cases.length);
		switch sw.cases[0] {
			case CaseBranch(b):
				Assert.equals(1, b.body.length);
				...
				switch b.patterns[0].expr {
					case Plain(Call(operand, args)):
						switch operand {
							...
							case null, _:
								Assert.fail('expected IdentExpr operand');
						}
	...
	public function testSwitchInModule(): Void {
		final module: HxModule = HaxeModuleParser.parse('class C { function f():Void { switch (x) { case 1: y; default: z; } } }');
		...
		switch stmts[0] {
			case SwitchStmt(sw):
				Assert.equals(2, sw.cases.length);
			case null, _:
				Assert.fail('expected SwitchStmt');
		}
	}

	public function testDefaultWithMultipleStatements(): Void {
		final sw: HxSwitchStmt = parseSwitch('class C { function f():Void { switch (x) { default: a; b; } } }');
		Assert.equals(1, sw.cases.length);
		switch sw.cases[0] {
			case DefaultBranch(branch):
				Assert.equals(2, branch.stmts.length);
			case null, _:
				Assert.fail('expected DefaultBranch');
		}
	}
	...
	public function testEmptyDefaultBody(): Void {
		...
		switch sw.cases[0] {
			case DefaultBranch(branch):
				Assert.equals(0, branch.stmts.length);
			case null, _:
				Assert.fail('expected DefaultBranch');
		}
	}
					...
					case NewExpr(ne):
						...
						switch ne.args[0] {
							case IntLit(v):
								Assert.equals(1, v);
							case null, _:
								Assert.fail('expected IntLit arg 0');
						}
						switch ne.args[1] {
	...
	public function testNewInSwitchBody(): Void {
		final sw: HxSwitchStmt = parseSwitch('class C { function f():Void { switch (x) { case 1: new Foo(); } } }');
		Assert.equals(1, sw.cases.length);
		switch sw.cases[0] {
			case CaseBranch(b):
				Assert.equals(1, b.body.length);
	...
	public function testNewInModule(): Void {
		final module: HxModule = HaxeModuleParser.parse('class C { function f():Void { new Foo(1); } }');
		Assert.equals(1, module.decls.length);
		final cls: HxClassDecl = expectClassDecl(module.decls[0]);
		Assert.equals(1, cls.members.length);
		final fn: HxFnDecl = expectFnMember(cls.members[0].member);
	...
	public function testNewQualifiedInSwitchBody(): Void {
		...
		final sw: HxSwitchStmt = parseSwitch('class C { function f():Void { switch (x) { case _: throw new haxe.Exception("oops"); } } }');
		Assert.equals(1, sw.cases.length);
		switch sw.cases[0] {
			case CaseBranch(b):
				Assert.equals(1, b.body.length);
	...
	public function testNewMultiTypeParams(): Void {
		final body: Array<HxStatement> =
			parseBody('class C { function f():Void { new Holder<RecordItem1, RecordItem1, Void>("a", true, "b"); } }');
		Assert.equals(1, body.length);
		switch body[0] {
			case ExprStmt(expr):
				switch expr {
	...
	public function testSwitchExprInCallArg(): Void {
		final body: Array<HxStatement> = parseBody('class C { function f():Void { trace(switch (x) { case 1: "a"; case _: "b"; }); } }');
		Assert.equals(1, body.length);
		switch body[0] {
			case ExprStmt(expr):
				switch expr {
