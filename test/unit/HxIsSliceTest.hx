...
class HxIsSliceTest extends HxTestHelpers {
	...
	public function testSimpleIs(): Void {
		...
		switch decl.init {
			...
			case null, _:
				Assert.fail('expected Is(IdentExpr, Named), got ${decl.init}');
		}
	}
	...
	public function testIsChainLeftAssoc(): Void {
		...
		switch decl.init {
			...
			case null, _:
				Assert.fail('expected Is(Is(_, _), _), got ${decl.init}');
		}
	}
	...
	public function testIdentNotConsumedAsIs(): Void {
		...
		switch decl.init {
			...
			case null, _:
				Assert.fail('expected Call(IdentExpr("is"), _), got ${decl.init}');
		}
	}
	...
	public function testIdentWithIsPrefix(): Void {
		...
		switch decl.init {
			case IdentExpr(name):
				Assert.equals('island', (name: String));
			case null, _:
				Assert.fail('expected IdentExpr("island"), got ${decl.init}');
		}
	}
