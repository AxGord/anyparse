...
class ApqDxTier5CliTest extends Test {
	...
	public function testRefsRuntimeHelperZeroHitExitsClean():Void {
		...
		Assert.equals(0, Cli.run(['refs', 'peekKw', 'src/anyparse/runtime/']),
			'refs on macro-emitted helper exits clean even with 0 hits');
	}
	...
	public function testRefsUnknownLowercaseNameStillExitsClean():Void {
		...
		Assert.equals(0, Cli.run(['refs', 'no_such_macro_helper_zzz', 'src/anyparse/runtime/']),
	...
	public function testAstSelectTypeNameNoMatchExitsClean():Void {
		...
		Assert.equals(0, Cli.run([
			'probe', 'class C {}', '--select', 'HxCatchClause'
		]),
	...
	public function testAstSelectLowercaseSelectorStillExitsClean():Void {
		...
		// fallback fires.
		Assert.equals(0, Cli.run([
			'probe', 'class C {}', '--select', 'unknownField'
		]),
	...
	public function testProbeStagesSourceToTmp():Void {
		...
		Assert.isTrue(FileSystem.exists(stagedPath),
			'probe stages source to $stagedPath');
		final staged:String = File.getContent(stagedPath);
		Assert.equals(source, staged,
			'staged file content matches the probe source byte-for-byte');
	}

	public function testProbeRestagingOverwritesPreviousScratch():Void {
		final stagedPath:String = '/tmp/anyparse-last-probe.hx';
		Assert.equals(0, Cli.run(['probe', 'class First {}']),
			'first probe exits clean');
		...
		Assert.equals(0, Cli.run(['probe', 'class Second { var b:Bool; }']),
			'second probe exits clean');
		Assert.equals('class Second { var b:Bool; }', File.getContent(stagedPath),
	...
	public function testReconCacheFileWritesOnEnvResolution():Void {
		...
		if (cacheStash != null) File.saveContent(cachePath, cacheStash);
	...
	public function testSelfStatusSourceFlagAccepted():Void {
		...
		Assert.equals(0, Cli.run(['self-status', '--source']),
			'self-status --source is a known flag');
	}

	public function testSelfStatusUnknownFlagStillRejected():Void {
		Assert.equals(2, Cli.run(['self-status', '--bogus']),
			'self-status rejects unknown flags as usage error');
	}
	...
	public function testSweepReadsCleanlyWithCurrentSnapshot():Void {
		...
		Assert.equals(0, Cli.run(['sweep']),
			'sweep exits clean with warn-check in the path');
	}
	#end
}
