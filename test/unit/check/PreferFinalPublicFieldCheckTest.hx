...
class PreferFinalPublicFieldCheckTest extends Test {
	 ...
	 * can no longer set. On the `-cs` target, `public var speed:Single = 200` and
	 ...
	 * what the emitted code does. Re-check the emitted output before changing this.
	 */
	...
	private function fixedSource(src: String): String {
		return CheckFixture.fixedSource(new PreferFinalPublicField(), src);
	}
