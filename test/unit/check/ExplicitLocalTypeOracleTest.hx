...
class ExplicitLocalTypeOracleTest extends Test {
	...
	public function testKeepsGeneric(): Void {
		Assert.equals('Array<Int>', LiteralInfer.normalizeInferredType('Array<Int>', [], 80));
	}

	public function testKeepsCleanFunctionType(): Void {
		Assert.equals('(x : Int) -> Void', LiteralInfer.normalizeInferredType('(x : Int) -> Void', [], 80));
	}

	public function testKeepsSmallAnon(): Void {
		Assert.equals('{ name : String }', LiteralInfer.normalizeInferredType('{ name : String }', [], 80));
	}

	// --- normalizeInferredType: rejection ---

	public function testRejectsMonomorphArray(): Void {
		Assert.isNull(LiteralInfer.normalizeInferredType('Array<Unknown<0>>', [], 80));
	}

	public function testRejectsMonomorphNull(): Void {
		Assert.isNull(LiteralInfer.normalizeInferredType('Null<Unknown<0>>', [], 80));
	}

	public function testRejectsFunctionHole(): Void {
		Assert.isNull(LiteralInfer.normalizeInferredType('(x : Unknown<0>) -> Unknown<0>', [], 80));
	}

	public function testRejectsVerboseAnonOverCap(): Void {
		Assert.isNull(LiteralInfer.normalizeInferredType('{ name : String, age : Int }', [], 10));
	}

	public function testRejectsBareUnderscore(): Void {
		Assert.isNull(LiteralInfer.normalizeInferredType('Class<_>', [], 80));
	}

	public function testRejectsEmpty(): Void {
		Assert.isNull(LiteralInfer.normalizeInferredType('   ', [], 80));
	}

	// --- stripTypeParamQualifiers: the compiler's qualified type parameters ---

	public function testStripsClassTypeParam(): Void {
		Assert.equals('T', LiteralInfer.stripTypeParamQualifiers('pkg.Box.T', { file: null, methodName: 'get' }));
	}

	public function testStripsClassTypeParamInsideGeneric(): Void {
		Assert.equals('pkg.Box<T>', LiteralInfer.stripTypeParamQualifiers('pkg.Box<pkg.Box.T>', { file: null, methodName: 'wrap' }));
	}

	public function testStripsMethodTypeParam(): Void {
		Assert.equals(
			'{ b : U, a : T }', LiteralInfer.stripTypeParamQualifiers('{ b : pair.U, a : pkg.Box.T }', { file: null, methodName: 'pair' })
		);
	}
	...
	public function testKeepsPackageQualifiedType(): Void {
		Assert.equals(
			...
			LiteralInfer.stripTypeParamQualifiers('haxe.ds.Map<String, Int>', { file: null, methodName: 'get' })
		);
	}

	public function testKeepsSecondaryModuleType(): Void {
		Assert.equals('pkg.Side', LiteralInfer.stripTypeParamQualifiers('pkg.Side', { file: null, methodName: 'side' }));
	}

	public function testKeepsMethodNameMismatch(): Void {
		Assert.equals('other.U', LiteralInfer.stripTypeParamQualifiers('other.U', { file: null, methodName: 'pair' }));
	}
	...
	public function testKeepsPackageTailMatchingNonGenericMethod(): Void {
		Assert.equals('utils.Thing', LiteralInfer.stripTypeParamQualifiers('utils.Thing', { file: null, methodName: null }));
	}
	...
	public function testStripsOwnPrivateModuleType(): Void {
		...
		Assert.equals('Array<Entry>', LiteralInfer.stripTypeParamQualifiers('Array<pkg._Holder.Entry>', site));
	}

	public function testKeepsForeignPrivateModuleType(): Void {
		final site: AnnotationSite = { file: 'pkg/Other.hx', methodName: null };
		Assert.equals('Array<pkg._Holder.Entry>', LiteralInfer.stripTypeParamQualifiers('Array<pkg._Holder.Entry>', site));
	}
	...
	public function testRejectsForeignPrivateModuleType(): Void {
		Assert.isNull(LiteralInfer.normalizeInferredType('Array<pkg._Holder.Entry>', [], 80));
	}

	public function testKeepsBareName(): Void {
		Assert.equals('String', LiteralInfer.stripTypeParamQualifiers('String', { file: null, methodName: 'get' }));
	}

	// --- normalizeInferredType: import-aware shortening ---

	public function testShortensBuiltinQualifiedMap(): Void {
		Assert.equals('Map<String, Int>', LiteralInfer.normalizeInferredType('haxe.ds.Map<String, Int>', [], 80));
	}

	public function testShortensImportedType(): Void {
		final imports: Map<String, String> = ['Foo' => 'pkg.Foo'];
		Assert.equals('Foo', LiteralInfer.normalizeInferredType('pkg.Foo', imports, 80));
	}

	public function testKeepsUnimportedFqn(): Void {
		Assert.equals('pkg.Foo', LiteralInfer.normalizeInferredType('pkg.Foo', [], 80));
	}

	public function testDoesNotMisShortenWrongFqn(): Void {
		final imports: Map<String, String> = ['Foo' => 'other.Foo'];
		Assert.equals('pkg.Foo', LiteralInfer.normalizeInferredType('pkg.Foo', imports, 80));
	}

	public function testShortensNestedGenericComponents(): Void {
		final imports: Map<String, String> = ['Foo' => 'pkg.Foo'];
		Assert.equals('Array<Foo>', LiteralInfer.normalizeInferredType('Array<pkg.Foo>', imports, 80));
	}
