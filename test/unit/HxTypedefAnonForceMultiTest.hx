...
class HxTypedefAnonForceMultiTest extends Test {
	...
	public function testFlatTypedefStaysFlatUnderSame():Void {
		...
		Assert.isTrue(out.indexOf('typedef T = {x:Int, y:Int, z:Int};') != -1,
			'expected flat typedef-RHS under Same in: <$out>');
	}
	...
	public function testFlatTypedefForcedMultiUnderNext():Void {
		...
		Assert.isTrue(out.indexOf('\n\tx:Int,\n\ty:Int,\n\tz:Int\n}') != -1,
			'expected one-per-line body in: <$out>');
	}
	...
	public function testFlatNonTypedefAnonNotForcedUnderNext():Void {
		...
		Assert.isTrue(out.indexOf('var a:{x:Int, y:Int};') != -1,
			'expected flat var-type-hint anon under Next in: <$out>');
	}
	...
	public function testIssue301Probe():Void {
		...
		Assert.isTrue(out.indexOf('typedef Point3D =\n{\n\tx:Int,\n\ty:Int,\n\tz:Int\n}') != -1,
