...
class ApqLitCommentsCliTest extends Test {
	...
	public function testLitIncludeCommentsFindsBlockCommentText():Void {
		#if sys
		final fixture:String = CliFixture.write('apq_lit_comments_block',
			'class C {\n\t/* the_marker_text lives here */\n\tvar x:Int = 0;\n}');
		Assert.equals(0, Cli.run(['lit', 'the_marker_text', '--include-comments', fixture]),
	...
	public function testLitIncludeCommentsFindsLineCommentText():Void {
		#if sys
		final fixture:String = CliFixture.write('apq_lit_comments_line',
	...
	public function testLitIncludeCommentsFindsDocCommentText():Void {
		#if sys
		final fixture:String = CliFixture.write('apq_lit_comments_doc',
	...
	public function testLitKindCommentScansOnlyComments():Void {
		...
		final fixture:String = CliFixture.write('apq_lit_kind_comment',
			'class C {\n\t/* the_marker_text comment */\n\tvar s:String = "the_marker_text string";\n}');
		Assert.equals(0, Cli.run(['lit', 'the_marker_text', '--kind', 'Comment', fixture]),
			'lit --kind Comment finds text in comment');
		FileSystem.deleteFile(fixture);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testLitDefaultSkipsCommentText():Void {
		...
		final fixture:String = CliFixture.write('apq_lit_default_skip',
			'class C {\n\t// only_in_comment lives here\n\tvar x:Int = 0;\n}');
		Assert.equals(0, Cli.run(['lit', 'only_in_comment', fixture]),
			'default lit (no --include-comments) skips comments');
		FileSystem.deleteFile(fixture);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testLitCommentScanIgnoresSlashSlashInString():Void {
		...
		final fixture:String = CliFixture.write('apq_lit_comment_string_aware',
			'class C { var s:String = "//not_a_comment"; }');
		Assert.equals(0, Cli.run(['lit', 'not_a_comment', '--kind', 'Comment', fixture]),
			'comment scan ignores // inside string literals');
		FileSystem.deleteFile(fixture);
		#else
		Assert.pass('non-sys target');
		#end
	...
	public function testLitAnyKindAlsoScansComments():Void {
		#if sys
		final fixture:String = CliFixture.write('apq_lit_anykind_comments',
			'class C {\n\t/* the_anykind_marker */\n\tvar x:Int = 0;\n}');
		Assert.equals(0, Cli.run(['lit', 'the_anykind_marker', '--any-kind', fixture]),
			'lit --any-kind scans comments alongside AST leaves');
		FileSystem.deleteFile(fixture);
		#else
		Assert.pass('non-sys target');
		#end
