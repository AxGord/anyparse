...
class StarBlockEndedTest extends Test {
	...
	public function testParseEmptyBlock():Void {
		...
		switch ast {
			case Block(items): Assert.equals(0, items.length);
			case _: Assert.fail('expected empty Block, got $ast');
		}
	}
	...
	public function testWriterAtomsHaveSep():Void {
		...
		Assert.isTrue(out.indexOf(';') >= 0,
			'expected sep `;` between atoms, got <$out>');
	}
	...
	public function testWriterNoSepAfterBlock():Void {
		...
		final out:String = MiniBlockWriter.write(
			Block([Block([Atom('a')]), Atom('b')])
		);
		...
		Assert.equals(-1, out.indexOf(';'),
			'expected no sep `;` after inner block, got <$out>');
	}
	...
	private function roundTrip(ast:MiniBlock):Void {
		...
		Assert.isTrue(equals(ast, reparsed),
			'round-trip mismatch: original=$ast, written=<$written>, reparsed=$reparsed');
	}

	private function assertAtom(node:MiniBlock, expected:String):Void {
		switch node {
			case Atom(s): Assert.equals(expected, (s : String));
			case _: Assert.fail('expected Atom($expected), got $node');
		}
	}
