...
class HxDoWhileThrowTryCatchSliceTest extends HxTestHelpers {
	...
	public function testThrowSmoke(): Void {
		...
		switch expr {
			...
			case null, _:
				Assert.fail('expected IdentExpr');
		}
	}
		...
		switch expr {
			case Add(left, right):
				switch left {
					...
					case null, _:
						Assert.fail('expected IdentExpr left');
				}
				switch right {
	...
	public function testThrowInModule(): Void {
		...
		switch stmts[0] {
			...
			case null, _:
				Assert.fail('expected ThrowStmt');
		}
	}
	...
	public function testDoWhileSmoke(): Void {
		...
		switch dw.body {
			...
			case null, _:
				Assert.fail('expected BlockBody body');
		}
		switch dw.cond {
			case IdentExpr(v):
	...
	public function testDoWhileSingleStatement(): Void {
		...
		switch dw.body {
			...
			case null, _:
				Assert.fail('expected ExprBody body');
		}
		switch dw.cond {
			case IdentExpr(v):
				Assert.equals('y', (v: String));
	...
	public function testDoWhileExpressionCond(): Void {
		...
		switch dw.body {
			...
			case null, _:
				Assert.fail('expected BlockBody');
		}
		switch dw.cond {
			case Add(left, right):
				switch left {
	...
	public function testDoWhileNested(): Void {
		final dw: HxDoWhileStmt = parseDoWhile('class C { function f():Void { do do x; while (a); while (b); } }');
		switch dw.body {
			case InnerDoWhile(inner):
				switch inner.body {
					...
					case null, _:
						Assert.fail('expected ExprBody inner body');
				}
				switch inner.cond {
			...
			case null, _:
				Assert.fail('expected InnerDoWhile body');
		}
		switch dw.cond {
			case IdentExpr(v):
				Assert.equals('b', (v: String));
			case null, _:
				Assert.fail('expected IdentExpr outer cond');
		}
	}
	...
	public function testDoWhileMacroBlockReification(): Void {
		final dw: HxDoWhileStmt = parseDoWhile("class C { function f():Void { do $block while (c); } }");
		switch dw.body {
			case ExprBody(expr):
				switch expr {
	...
	public function testDoWhileBarePostIncr(): Void {
		final dw: HxDoWhileStmt = parseDoWhile('class C { function f():Void { do a++ while (true); } }');
		switch dw.body {
			case ExprBody(expr):
				switch expr {
	...
	public function testTryCatchSmoke(): Void {
		final tc: HxTryCatchStmt = parseTryCatch('class C { function f():Void { try { x; } catch (e:E) { y; } } }');
		switch tc.body {
			case BlockStmt(stmts):
				Assert.equals(1, stmts.length);
				switch stmts[0] {
			...
			case null, _:
				Assert.fail('expected BlockStmt body');
		}
		...
		switch c.body {
			case BlockStmt(stmts):
				Assert.equals(1, stmts.length);
			case null, _:
				Assert.fail('expected BlockStmt catch body');
		}
	}
	...
	public function testTryCatchSingleStatement(): Void {
		...
		switch tc.body {
			...
			case null, _:
				Assert.fail('expected ExprStmt body');
		}
		Assert.equals(1, tc.catches.length);
		switch tc.catches[0].body {
			...
			case null, _:
				Assert.fail('expected ExprStmt catch body');
		}
	}

	public function testMultipleCatches(): Void {
		final tc: HxTryCatchStmt = parseTryCatch('class C { function f():Void { try { } catch (e1:E1) { a; } catch (e2:E2) { b; } } }');
	...
	public function testTryCatchNested(): Void {
		final tc: HxTryCatchStmt = parseTryCatch('class C { function f():Void { try { } catch (e:E) { try { } catch (e2:E2) { } } } }');
		Assert.equals(1, tc.catches.length);
		switch tc.catches[0].body {
			case BlockStmt(stmts):
				Assert.equals(1, stmts.length);
	...
	public function testTryCatchInModule(): Void {
		final module: HxModule = HaxeModuleParser.parse('class C { function f():Void { try { } catch (ex:Exception) { } } }');
		Assert.equals(1, module.decls.length);
		final cls: HxClassDecl = expectClassDecl(module.decls[0]);
		final fn: HxFnDecl = expectFnMember(cls.members[0].member);
		final stmts: Array<HxStatement> = fnBodyStmts(fn);
		...
		switch stmts[0] {
			...
			case null, _:
				Assert.fail('expected TryCatchStmt');
		}
	}

	public function testTryCatchWhitespace(): Void {
		final tc: HxTryCatchStmt = parseTryCatch('class C { function f():Void { try  {  }  catch  (  e  :  E  )  {  }  } }');
		Assert.equals(1, tc.catches.length);
	...
	public function testWordBoundaryCatching(): Void {
		final tc: HxTryCatchStmt = parseTryCatch('class C { function f():Void { try { catching; } catch (e:E) { } } }');
		switch tc.body {
			case BlockStmt(stmts):
				Assert.equals(1, stmts.length);
				switch stmts[0] {
	...
	public function testThrowInCatchBody(): Void {
		final tc: HxTryCatchStmt = parseTryCatch('class C { function f():Void { try { } catch (e:E) { throw e; } } }');
		Assert.equals(1, tc.catches.length);
		switch tc.catches[0].body {
			case BlockStmt(stmts):
				Assert.equals(1, stmts.length);
