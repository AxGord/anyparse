...
final class HxCallParamNestedArrayHugSliceTest extends Test {
	...
	public function testNestedArrayTailOverflowStillHugs(): Void {
		final src: String = 'class C {\n\tfunction test() {\n\t\tcontainer.addItem(new Row([\n'
			+ '\t\t\tnew Label("alphaLongText", fmtFunctionOne(), null, 30),\n\t\t\tnew Label("bravoLongText", fmtFunctionTwo(), null, '
	...
	public function testDirectArrayArgStillHugs(): Void {
		final src: String = 'class C {\n\tfunction test() {\n\t\tcontainer.addItem([\n\t\t\tnew Label("alpha", fmtOneFunctionCall(), null, '
			+ '30),\n\t\t\tnew Label("bravo", fmtTwoFunctionCall(), null, 30)\n\t\t], false);\n\t}\n}';
		Assert.equals(src, triviaWrite(src));
	}

	public function testNestedCallNoArrayLeadingBreaks(): Void {
		final src: String = 'class C {\n\tfunction test() {\n\t\tcontainer.addItem(\n\t\t\tnew Row(longArgumentAlphaValue, '
			+ 'longArgumentBravoValue, longArgumentCharlieValue, longArgumentDeltaValue, longEchoValue), false\n\t\t);\n\t}\n}';
		Assert.equals(src, triviaWrite(src));
	}
	...
	public function testChainOwnedArrayPastSoftBreakLeadingBreaks(): Void {
		final src: String = 'class C {\n\tfunction test() {\n\t\treturn assert(\n\t\t\talphaCount == 0,\n'
			+ '\t\t\t\'Should not create items for the given additions, but created $${alphaCount}: \' + [for (m in moves) '
			+ '\'$${m.oldPath}\'].join(\', \')\n\t\t);\n\t}\n}';
		Assert.equals(src, triviaWrite(src));
	}
