...
class HxFnReturnBodySliceTest extends HxTestHelpers {
	...
	public function testClassMemberReturnBodyComplex(): Void {
		...
		switch inner {
			case Add(_, _):
				Assert.pass();
			case _:
				Assert.fail('expected return value Add(_, _), got $inner');
		}
	}
	...
	public function testToplevelFnReturnBody(): Void {
		...
		switch ast.decls[0].decl {
			case FnDecl(decl):
				assertReturnExprBody(decl.body);
			case _:
	...
	private function assertReturnExprBody(body: HxFnBody): Void {
		switch body {
			case ExprBody(ReturnExpr(_)):
				Assert.pass();
			case _:
				Assert.fail('expected ExprBody(ReturnExpr(...)), got $body');
		}
	}
