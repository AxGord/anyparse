...
final class HxTernaryDeclInitRestAwareTest extends Test {
	...
	private static final CFG: String = '{"indentation":{"character":"tab","tabWidth":4,"trailingWhitespace":false,'
		...
		+ '"functionSignature":{"defaultWrap":"fillLineWithLeadingBreak",'
		...
		+ '"switchSubjectParens":"remove"}},"lineEnds":{"emptyCurly":"noBreak"},'
	...
	public function testTernaryDeclInitWrapsWhenPendingSpaceOverflows(): Void {
		Assert.equals(
			...
			triviaWrite(
				'class Sample {\n\tfunction run() {\n\t\tfinal resolvedStamp:Int = syncResult.Success && syncResult.Payload != null ? '
				+ 'syncResult.Payload.LastModifiedStamp : fallbackStampXXX;\n\t}\n}',
				CFG
			)
		);
	}
	...
	public function testTernaryDeclInitFlatAtExactLimit(): Void {
		Assert.equals(
			'class Sample {\n\n\tfunction run() {\n\t\tfinal resolvedStamp:Int = syncResult.Success && syncResult.Payload != null ? '
			+ 'syncResult.Payload.LastModifiedStamp : fallbackStampXX;\n\t}\n\n}',
			triviaWrite(
				'class Sample {\n\tfunction run() {\n\t\tfinal resolvedStamp:Int = syncResult.Success && syncResult.Payload != null ? '
				+ 'syncResult.Payload.LastModifiedStamp : fallbackStampXX;\n\t}\n}',
				CFG
